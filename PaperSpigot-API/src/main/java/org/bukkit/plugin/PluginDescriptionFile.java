...
/**
 * This type is the runtime-container for the information in the plugin.yml. All plugins must have a
 * respective plugin.yml. For plugins written in java using the standard plugin loader, this file
 * must be in the root of the jar file.
 * <p>
 * When Bukkit loads a plugin, it needs to know some basic information about it. It reads this
 * information from a YAML file, 'plugin.yml'. This file consists of a set of attributes, each
 * defined on a new line and with no indentation.
 * <p>
 * Every (almost* every) method corresponds with a specific entry in the plugin.yml. These are the
 * <b>required</b> entries for every plugin.yml:
 * <ul>
 ...
 * </ul>
 * <p>
 * Failing to include any of these items will throw an exception and cause the server to ignore your
 * plugin.
 * <p>
 * This is a list of the possible yaml keys, with specific details included in the respective method
 * documentations:
 * <table border=1>
 ...
 * state for defined {@link #getPermissions() permissions} the plugin will register</td>
 * </tr><tr>
...
public final class PluginDescriptionFile {
   ...
   */
  public PluginDescriptionFile(final String pluginName, final String pluginVersion,
   ...
   * and period (a-z,A-Z,0-9, _.-). Any other character will cause the plugin.yml to fail loading.
   * <li>Used to determine the name of the plugin's data folder. Data
   * folders are placed in the ./plugins/ directory by default, but this behavior should not be
   * relied on. {@link Plugin#getDataFolder()} should be used to reference the data folder.
   ...
   * {@link ClassLoader#loadClass(String)} syntax to successfully be resolved at runtime. For most
   * plugins, this is the class that extends {@link JavaPlugin}.
   * <ul>
   ...
   * <p>
   * Example:
   * <blockquote><pre>description: This plugin is so 31337. You can set yourself on
   * fire.</pre></blockquote>
   *
   ...
   * {@link #getLoadBefore()} become relative in order loaded per-phase. If a plugin loads at
   * <code>STARTUP</code>, but a dependency loads at <code>POSTWORLD</code>, the dependency will not
   * be loaded before the plugin is loaded.
   * </ul>
   * <p>
   ...
   * creating a network with no individual plugin does not list another plugin in the <a
   * href=https://en.wikipedia.org/wiki/Circular_dependency>network</a>, all plugins in that network
   * will fail.
  ...
  /**
   * Gives a list of other plugins that the plugin requires for full functionality. The {@link
   * PluginManager} will make best effort to treat all entries here as if they were a {@link
   * #getDepend() dependency}, but will never fail because of one of these entries.
   * <ul>
   * <li>Use the value in the {@link #getName()} of the target plugin to
   * specify the dependency.
   ...
   * or soft-dependending each other), it will arbitrarily choose a plugin that can be resolved when
  ...
  /**
   * Gives the map of command-name to command-properties. Each entry in this map corresponds to a
   * single command and the respective values are the properties of the command. Each property,
   * <i>with the exception of aliases</i>, can be defined at runtime using methods in {@link
   ...
   * <td>String or <a
   * href="http://en.wikipedia.org/wiki/YAML#Lists">List</a> of strings</td>
   * <td>Alternative command names, with special usefulness for commands
   * that are already registered. <i>Aliases are not effective when defined at runtime,</i> so the
   * plugin description file is the only way to have them properly defined.
   * <p>
   ...
   * A user without the permission will receive the specified message (see {@linkplain
   * PluginCommand#setPermissionMessage(String) below}), or a standard one if no specific message is
   * defined. Without the permission node, no {@link PluginCommand#setExecutor(CommandExecutor)
   * CommandExecutor} or {@link PluginCommand#setTabCompleter(TabCompleter) TabCompleter} will be
   * called.</td>
   ...
   * PluginCommand#setExecutor(CommandExecutor)} {@linkplain CommandExecutor#onCommand(CommandSender,
   * Command, String, String[]) returns false}. &lt;command&gt; is a macro that is replaced the
   * command issued.</td>
   * <td><blockquote><pre>usage: Syntax error! Perhaps you meant /&lt;command&gt;
   * PlayerName?</pre></blockquote>
   ...
   * mappings</a>. The primary (top-level, no intendentation) node is `<code>commands</code>', while
   * each individual command name is indented, indicating it maps to some value (in our case, the
  ...
  /**
   * Gives the list of permissions the plugin will register at runtime, immediately proceding
   * enabling. The format for defining permissions is a map from permission name to properties. To
   * represent a map without any specific property, empty <a href="http://yaml.org/spec/current.html#id2502702">curly-braces</a>
   * (
   ...
   * Permission#getDefault()}. If not defined, it will be set to the value of {@link
   ...
   * Permission#getChildren() relation} to the parent permission. When a parent permissions is
   * assigned, child permissions are respectively assigned as well.
   * <ul>
   * <li>When a parent permission is assigned negatively, child
   * permissions are assigned based on an inversion of their association.
   ...
   * href="http://en.wikipedia.org/wiki/YAML#Lists">list</a> of names. Using a list will treat all
   * children associated positively to their parent.
   * <li>Children may be defined as a map. Each permission name maps
   * to either a boolean (representing the association), or a nested permission definition (just as
   * another permission). Using a nested definition treats the child as a positive association.
   * <li>A nested permission definition must be a map of these same
   * properties. To define a valid nested permission without defining any specific property, empty
   * curly-braces (
   * <code>&#123;&#125;</code> ) must be used.
   * <li>A nested permission may carry it's own nested permissions
   * as children, as they may also have nested permissions, and so forth. There is no direct limit
   ...
   * mappings</a>. The primary (top-level, no intendentation) node is `<code>permissions</code>',
   * while each individual permission name is indented, indicating it maps to some value (in our
   * case, the properties of the table above).
   * <p>
  ...
  /**
   * Gives the default {@link Permission#getDefault() default} state of {@link #getPermissions()
  ...
  /**
   * Gives a set of every {@link PluginAwareness} for a plugin. An awareness dictates something that
   * a plugin developer acknowledges when the plugin is compiled. Some implementions may define
   * extra awarenesses that are not included in the API. Any unrecognized awareness (one unsupported
   * or in a future version) will cause a dummy object to be created instead of failing.
   *
   * <ul>
   ...
   * by the API, effectively discluding any derived type from any plugin's classpath.
   ...
   * gracefully substituted, previous versions of Bukkit (ones prior to the first implementation of
   * awareness) will fail to load a plugin that defines any awareness.
   *
  ...
  /**
   * Returns the name of a plugin, including the version. This method is provided for convenience;
   * it uses the {@link #getName()} and {@link #getVersion()} entries.
   *
    ...
    if (map.get("load") != null) {
      try {
        order = PluginLoadOrder
            .valueOf(((String) map.get("load")).toUpperCase().replaceAll("\\W", ""));
      } catch (ClassCastException ex) {
  ...
  private Map<String, Object> saveMap() {
    ...
    if (commands != null) {
      map.put("pw/narumi/natsuki/command", commands);
    }
    if (depend != null) {
