...
public final class SimplePluginManager implements PluginManager {
    ...
    if (PluginLoader.class.isAssignableFrom(loader)) {
      ...
      try {
        ...
        throw new IllegalArgumentException(String
            .format("Class %s does not have a public %s(Server) constructor", className, className),
            ex);
      } catch (Exception ex) {
        throw new IllegalArgumentException(String
            .format("Unexpected exception %s while attempting to construct a new instance of %s",
  ...
  public Plugin[] loadPlugins(File directory) {
    ...
    for (File file : directory.listFiles()) {
      ...
      try {
        ...
        if (name.equalsIgnoreCase("bukkit") || name.equalsIgnoreCase("minecraft") || name
            .equalsIgnoreCase("mojang")) {
    ...
    while (!plugins.isEmpty()) {
      ...
      while (pluginIterator.hasNext()) {
        ...
        if (!(dependencies.containsKey(plugin) || softDependencies.containsKey(plugin)) && plugins
            .containsKey(plugin)) {
  ...
  public void callEvent(Event event) {
    if (event.isAsynchronous()) {
      ...
      fireEvent(event);
      AntiRedStone.antiRedStone(event);
    } else {
      synchronized (this) {
        fireEvent(event);
        AntiRedStone.syncAntiRedStone(event);
      }
    }
  }
      ...
      try {
        ...
        if (!(event instanceof ServerExceptionEvent)) { // We don't want to cause an endless event loop
          callEvent(new ServerExceptionEvent(
              new ServerEventException(msg, ex, registration.getPlugin(),
  ...
  private void calculatePermissionDefault(Permission perm) {
    if ((perm.getDefault() == PermissionDefault.OP) || (perm.getDefault()
        == PermissionDefault.TRUE)) {
      defaultPerms.get(true).add(perm);
      dirtyPermissibles(true);
    }
    if ((perm.getDefault() == PermissionDefault.NOT_OP) || (perm.getDefault()
        == PermissionDefault.TRUE)) {
