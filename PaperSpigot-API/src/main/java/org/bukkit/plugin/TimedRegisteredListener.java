...
public class TimedRegisteredListener extends RegisteredListener {
  ...
  public TimedRegisteredListener(final Listener pluginListener, final EventExecutor eventExecutor,
      final EventPriority eventPriority, final Plugin registeredPlugin,
      final boolean listenCancelled) {
   ...
   * the closest shared superclass will be returned, such that for any event this listener has
   * handled,
   * <code>this.getEventClass().isAssignableFrom(event.getClass())</code>
   * and no class <code>this.getEventClass().isAssignableFrom(clazz) {@literal &&
   * this.getEventClass() != clazz &&} event.getClass().isAssignableFrom(clazz)</code> for all
   * handled events.
   *
