...
public final class JavaPluginLoader implements PluginLoader {
  ...
  public Plugin loadPlugin(final File file) throws InvalidPluginException {
    ...
    if (!file.exists()) {
      throw new InvalidPluginException(
          new FileNotFoundException(file.getPath() + " does not exist"));
    }
    ...
    @SuppressWarnings("deprecation") final File oldDataFolder = new File(parentFile,
    ...
    try {
      loader = new PluginClassLoader(this, getClass().getClassLoader(), description, dataFolder,
          file);
  ...
  public Map<Class<? extends Event>, Set<RegisteredListener>> createRegisteredListeners(
      Listener listener, final Plugin plugin) {
    ...
    for (final Method method : methods) {
      ...
      if (method.getParameterTypes().length != 1 || !Event.class
        ...
        plugin.getLogger().severe(plugin.getDescription().getFullName()
            + " attempted to register an invalid EventHandler method signature \"" + method
  ...
  public void enablePlugin(final Plugin plugin) {
    Validate
        .isTrue(plugin instanceof JavaPlugin, "Plugin is not associated with this PluginLoader");
