...
public abstract class JavaPlugin extends PluginBase {
   ...
   */
  @Deprecated
  protected JavaPlugin(final PluginLoader loader, final Server server,
  ...
  protected JavaPlugin(final JavaPluginLoader loader, final PluginDescriptionFile description,
  ...
  /**
   * Provides a reader for a text file located inside the jar. The behavior of this method adheres
   * to {@link PluginAwareness.Flags#UTF8}, or if not defined, uses UTF8 if {@link
   * FileConfiguration#UTF8_OVERRIDE} is specified, or system default otherwise.
   *
  ...
  protected final Reader getTextResource(String file) {
    ...
    return in == null ? null : new InputStreamReader(in,
        isStrictlyUTF8() || FileConfiguration.UTF8_OVERRIDE ? Charsets.UTF_8
    ...
    try {
      if (!outFile.exists() || replace) {
        ...
        logger.log(Level.WARNING,
            "Could not save " + outFile.getName() + " to " + outFile + " because " + outFile
   ...
   * @deprecated This method is legacy and will be removed - it must be replaced by the specially
   * provided constructor(s).
   */
  @Deprecated
  ...
  final void init(PluginLoader loader, Server server, PluginDescriptionFile description,
   ...
   * @deprecated This method cannot return false, as {@link JavaPlugin} is now initialized in the
   * constructor.
   */
  @Deprecated
   ...
   * in the {@link PluginDescriptionFile#getCommands() PluginDescriptionFile} to exist at runtime.
   *
  ...
  /**
   * This method provides fast access to the plugin that has {@link #getProvidingPlugin(Class)
   * provided} the given plugin class, which is usually the plugin that implemented it.
   * <p>
   * An exception to this would be if plugin's jar that contained the class does not extend the
   * class, where the intended plugin would have resided in a different jar / classloader.
   *
   ...
   * @throws IllegalStateException if clazz was not provided by a plugin, for example, if called
   * with
  ...
  public static <T extends JavaPlugin> T getPlugin(Class<T> clazz) {
    ...
    if (!(cl instanceof PluginClassLoader)) {
      throw new IllegalArgumentException(
          clazz + " is not initialized by " + PluginClassLoader.class);
    }
    JavaPlugin plugin = ((PluginClassLoader) cl).plugin;
    if (plugin == null) {
      throw new IllegalStateException(
          "Cannot get plugin for " + clazz + " from a static initializer");
    }
