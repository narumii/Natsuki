...
public interface World extends PluginMessageRecipient, Metadatable {
  ...
  /**
   * Checks if the {@link Chunk} at the specified coordinates is loaded and in use by one or more
   * players
   *
   * @param x X-coordinate of the chunk
   * @param z Z-coordinate of the chunk
   ...
   * <p>
   * This method is analogous to {@link #unloadChunk(int, int, boolean, boolean)} where safe and
   * saveis true
   *
   ...
   * <p>
   * This method is analogous to {@link #unloadChunk(int, int, boolean, boolean)} where save is
   * true
   *
   * @param x X-coordinate of the chunk
   * @param z Z-coordinate of the chunk
   ...
   * @deprecated Has issues spawning non LivingEntities. Use {@link #spawnEntity(Location,
   * EntityType) spawnEntity} instead.
   */
  @Deprecated
   ...
   * @return A List of all Entities currently residing in this world that match the given
   * class/interface
   */
  @Deprecated
   ...
   *
   * @param classes The classes representing the types of entity to match
   * @return A List of all Entities currently residing in this world that match one or more of the
   ...
   * The relative time is analogous to hours * 1000
   * <p>
   * Note that setting the relative time below the current relative time will actually move the
   * clock forward a day. If you require to rewind time, please see {@link #setFullTime(long)}
   *
   ...
   * <p>
   * Note that this sets the full time of the world, which may cause adverse effects such as
   * breaking redstone clocks and any scheduled events
   *
   ...
   * @throws IllegalArgumentException if either parameter is null or the {@link Entity} requested
   * cannot be spawned
   */
  <T extends Entity> T spawn(Location location, Class<T> clazz) throws IllegalArgumentException;

  /**
   * Spawn a {@link FallingBlock} entity at the given {@link Location} of the specified {@link
   * Material}. The material dictates what is falling. When the FallingBlock hits the ground, it
   * will place that block.
   * <p>
   ...
   * @throws IllegalArgumentException if {@link Location} or {@link Material} are null or {@link
   * Material} is not a block
   * @deprecated Magic value
   */
  @Deprecated
  ...
  /**
   * Spawn a {@link FallingBlock} entity at the given {@link Location} of the specified blockId
   * (converted to {@link Material})
   *
   * @param location The {@link Location} to spawn the FallingBlock
   ...
   * Used for representing an ungenerated chunk, or for fetching only biome data without loading a
   * chunk.
   *
   * @param x - chunk x coordinate
   * @param z - chunk z coordinate
   * @param includeBiome - if true, snapshot includes per-coordinate biome type
   * @param includeBiomeTempRain - if true, snapshot includes per-coordinate raw biome temperature
   * and rainfall
   ...
   * <p>
   * <b>Note:</b>
   * If set to 0, animal spawning will be disabled for this world. We recommend using {@link
   ...
   * <p>
   * <b>Note:</b>
   * If set to 0, monsters spawning will be disabled for this world. We recommend using {@link
   * #setSpawnFlags(boolean, boolean)} to control this instead.
   * <p>
   * Minecraft default: 1.
   *
  ...
  class Spigot {
     ...
     * @param effect the {@link Effect}
     * @throws IllegalArgumentException if the location or effect is null. It also throws when the
     * effect requires a material or a material data
     */
    ...
    /**
     * Plays an effect to all players within a default radius around a given location. The effect
     * will use the provided material (and material data if required). The particle's position on
     * the client will be the given location, adjusted on each axis by a normal distribution with
     * mean 0 and standard deviation given in the offset parameters, each particle has independently
     * calculated offsets. The effect will have the given speed and particle count if the effect is
     * a particle. Some effect will create multiple particles.
     *
     * @param location the {@link Location} around which players must be to see the effect
    ...
    /**
     * Strikes lightning at the given {@link Location} without doing damage and possibly without
     * sound
     *
     * @param loc The location to strike lightning
     * @param isSilent Whether this strike makes no sound
