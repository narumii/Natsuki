...
public class BlockIterator implements Iterator<Block> {
   ...
   * @param maxDistance This is the maximum distance in blocks for the trace. Setting this value
   * above 140 may lead to problems with unloaded chunks. A value of 0 indicates no limit
   */
  public BlockIterator(World world, Vector start, Vector direction, double yOffset,
      int maxDistance) {
    this.world = world;
    ...
    Block startBlock = this.world
        .getBlockAt(floor(startClone.getX()), floor(startClone.getY()), floor(startClone.getZ()));
    ...
    maxDistanceInt = round(maxDistance / (Math.sqrt(
        mainDirection * mainDirection + secondDirection * secondDirection
