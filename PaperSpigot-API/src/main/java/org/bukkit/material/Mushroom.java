...
public class Mushroom extends MaterialData {
  ...
  public Mushroom(Material shroom) {
    super(shroom);
    Validate.isTrue(shroom == Material.HUGE_MUSHROOM_1 || shroom == Material.HUGE_MUSHROOM_2,
        "Not a mushroom!");
  }
  ...
  public Mushroom(int type, byte data) {
    super(type, data);
    Validate.isTrue(
        type == Material.HUGE_MUSHROOM_1.getId() || type == Material.HUGE_MUSHROOM_2.getId(),
        "Not a mushroom!");
  }
  ...
  /**
   * Set a face of the block to be painted or not. Note that due to the nature of how the data is
   * stored, setting a face painted or not is not guaranteed to leave the other faces unchanged.
   *
  ...
  public String toString() {
    return Material.getMaterial(getItemTypeId()).toString() + (isStem() ? "{STEM}"
        : getPaintedFaces());
  }
