...
public interface Server extends PluginMessageRecipient {
   ...
   * Collections#unmodifiableCollection(Collection) view} is a reused object, making some operations
   * like {@link Collection#size()} zero-allocation.
   * <p>
   * The collection is a view backed by the internal representation, such that, changes to the
   * internal state of the server will be reflected immediately. However, the reuse of the returned
   * collection (identity) is not strictly guaranteed for future or all implementations. Casting the
   * collection, or relying on interface implementations (like {@link Serializable} or {@link
   * List}), is deprecated.
   * <p>
   * Iteration behavior is undefined outside of self-contained main-thread uses. Normal and
   * immediate iterator use without consequences that affect the collection are fully supported. The
   * effects following (non-exhaustive) {@link Entity#teleport(Location) teleportation}, {@link
   * Player#setHealth(double) death}, and {@link Player#kickPlayer(String) kicking} are undefined.
   * Any use of this collection from asynchronous threads is unsafe.
   * <p>
   * For safe consequential iteration or mimicking the old array behavior, using {@link
   * Collection#toArray(Object[])} is recommended. For making snapshots, {@link
   * ImmutableList#copyOf(Collection)} is recommended.
   *
  ...
  /**
   * Get an ID of this server. The ID is a simple generally alphanumeric ID that can be used for
   * uniquely identifying this server.
   *
  ...
  /**
   * Gets the name of the update folder. The update folder is used to safely update plugins at the
   * right moment on a plugin load.
   * <p>
  ...
  /**
   * Gets the update folder. The update folder is used to safely update plugins at the right moment
   * on a plugin load.
   *
   ...
   * <p>
   * This list is not sorted in any particular order. If an exact match is found, the returned list
   ...
   * @throws CommandException thrown when the executor for the given command fails with an unhandled
   * exception
   */
  ...
  /**
   * Get a list of all recipes for a given item. The stack size is ignored in comparisons. If the
   * durability is -1, it will match any data value.
   *
   ...
   * </ul>
   *
   * @return true if exact location locations are used for spawning, false for vanilla collision
   ...
   * @param permission the required permission {@link Permissible permissibles} must have to receive
   * the broadcast
   ...
   * <p>
   * This will return an object even if the player does not exist. To this method, all players will
   * exist.
   *
   * @param name the name the player to retrieve
   * @return an offline player
   * @see #getOfflinePlayer(java.util.UUID)
   * @deprecated Persistent storage of users should be by UUID as names are no longer unique past a
   * single session.
   */
  @Deprecated
   ...
   * <p>
   * Bans by name are no longer supported and this method will return null when trying to request
   * them. The replacement is bans by UUID.
   *
   ...
   * the new inventory has a size of 27; otherwise the new inventory has the normal size for its
   * type.
   *
  ...
  /**
   * Creates an empty inventory with the specified type and title. If the type is {@link
   * InventoryType#CHEST}, the new inventory has a size of 27; otherwise the new inventory has the
   * normal size for its type.<br> It should be noted that some inventory types do not support
   * titles and may not render with said titles on the Minecraft client.
   *
  ...
  /**
   * Creates an empty inventory of type {@link InventoryType#CHEST} with the specified size and
   * title.
   *
   * @param owner the holder of the inventory, or null to indicate no holder
   * @param size a multiple of 9 as the size of inventory to create
   ...
   * synchronized state of the runtime. A current thread matching the main thread indicates that it
   * is synchronized, but a mismatch <b>does not preclude</b> the same assumption.
   *
   ...
   *
   * @return the default server-icon; null values may be used by the implementation to indicate no
   * defined icon, but this behavior is not guaranteed
   */
   ...
   * <p>
   * Size and type are implementation defined. An incompatible file is guaranteed to throw an
   * implementation-defined {@link Exception}.
   *
  ...
  /**
   * Set the idle kick timeout. Any players idle for the specified amount of time will be
  ...
  class Spigot {
    ...
    /**
     * Restart the server. If the server administrator has not configured restarting, the server
     * will stop.
     */
    public void restart() {
      throw new UnsupportedOperationException("Not supported yet.");
