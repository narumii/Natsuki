 ...
 */
public interface Player extends HumanEntity, Conversable, CommandSender, OfflinePlayer,
    PluginMessageRecipient {

  boolean connectionsVisible();

  void setConnectionsVisible(final boolean connectionsVisible);
   ...
   * <p>
   * This name is case sensitive and unique, two names with different casing will appear as two
   * different people. If a player joins afterwards with a name that conflicts with a player's
   * custom list name, the joining player's player list name will have a random number appended to
   * it (1-2 characters long in the default implementation). If the joining player's name is 15 or
   * 16 characters long, part of the name will be truncated at the end to allow the addition of the
   * two digits.
   *
  ...
  /**
   * Loads the players current location, health, inventory, motion, and other information from the
   * username.dat file, in the world/player folder.
   * <p>
   * Note: This will overwrite the players current inventory, health, motion, etc, with the state
   * from the saved dat file.
   */
  void loadData();

  /**
   * Sets whether the player is ignored as not sleeping. If everyone is either sleeping or has this
   * flag set, then time will advance to the next day. If everyone has this flag set but no one is
   * actually in bed, then nothing will happen.
   *
  ...
  /**
   * Play a note for a player at a location. This requires a note block at the particular location
   * (as far as the client is concerned). This will not work without a note block. This will not
   * work with cake.
   *
   ...
   * Play a sound for a player at the location.
   * <p>
   * This function will fail silently if Location or Sound are null. No sound will be heard by the
   * player if their client does not have the respective sound for the value passed.
   *
   * @param location the location to play the sound
  ...
  /**
   * Send a block change. This fakes a block change packet for a user at a certain location. This
  ...
  /**
   * Send a chunk change. This fakes a chunk change packet for a user at a certain location. The
   * updated cuboid must be entirely within a single chunk. This will not actually change the world
   * in any way.
   * <p>
   * At least one of the dimensions of the cuboid must be even. The size of the data buffer must be
   * 2.5*sx*sy*sz and formatted in accordance with the Packet51 format.
   *
  ...
  /**
   * Send a sign change. This fakes a sign change packet for a user at a certain location. This will
   * not actually change the world in any way. This method will use a sign at the location's block
   * or a faked sign sent via {@link #sendBlockChange(org.bukkit.Location, int, byte)} or {@link
   * #sendBlockChange(org.bukkit.Location, org.bukkit.Material, byte)}.
   * <p>
   * If the client does not have a sign at the given location it will display an error message to
   * the user.
   *
  ...
  /**
   * Render a map and send it to the player in its entirety. This may be used when streaming the map
   * in the normal manner is not desirable.
   *
  ...
  @Deprecated
  void showTitle(net.md_5.bungee.api.chat.BaseComponent[] title,
      net.md_5.bungee.api.chat.BaseComponent[] subtitle, int fadeInTicks, int stayTicks,
      int fadeOutTicks);
   ...
   *
   * <p>This method overrides any previous title, use {@link #updateTitle(Title)} to change the
   * existing one.</p>
   *
   ...
   * @throws IllegalArgumentException if the statistic requires an additional parameter
   */
   ...
   * @throws IllegalArgumentException if the given parameter is not valid for the statistic
   */
  ...
  /**
   * Sets the current time on the player's client. When relative is true the player's time will be
   * kept synchronized to its world time with the specified offset.
   * <p>
   * When using non relative time the player's time will stay fixed at the specified time parameter.
   * It's up to the caller to continue updating the player's time. To restore player time to normal
   * use resetPlayerTime().
   *
   * @param time The current player's perceived time or the player's time offset from the server
   * time.
  ...
  /**
   * Returns the player's current time offset relative to server time, or the current player's fixed
   * time if the player's time is absolute.
   *
   * @return The player's time
   */
  ...
  /**
   * Returns true if the player's time is relative to the server time, otherwise the player's time
   * is absolute and will not change its current time unless done so with setPlayerTime().
   *
  ...
  /**
   * Sets the type of weather the player will see.  When used, the weather status of the player is
   * locked until {@link #resetPlayerWeather()} is used.
   *
   ...
   *
   * @return The WeatherType that the player is currently experiencing or null if player is seeing
   * server weather.
   */
  ...
  /**
   * Gives the player the amount of experience levels specified. Levels can be taken by specifying a
   * negative amount.
   *
   ...
   * <p>
   * Exhaustion controls how fast the food level drops. While you have a certain amount of
   * exhaustion, your saturation will drop to zero, and then your food will drop to zero.
   *
   ...
   * <p>
   * Saturation is a buffer for food level. Your food level will not drop if you are saturated
   * {@literal >} 0.
   *
   ...
   * or their current bed spawn is invalid.
   *
  ...
  /**
   * Checks to see if this player is currently standing on a block. This information may not be
   * reliable, as it is a state provided by the client, and may therefore not be accurate.
   *
   ...
   * will automatically switch to it once the download is complete. If the client has downloaded and
   * cached the same texture pack in the past, it will perform a quick timestamp check over the
   * network to determine if the texture pack has changed and needs to be downloaded again. When
   * this request is sent for the very first time from a given server, the client will first display
   * a confirmation GUI to the player before proceeding with the download.
   * <p>
   * Notes:
   * <ul>
   ...
   * </ul>
   *
   * @param url The URL from which the client will download the texture pack. The string must
   * contain only US-ASCII characters and should be encoded as per RFC 1738.
   * @throws IllegalArgumentException Thrown if the URL is null.
   * @throws IllegalArgumentException Thrown if the URL is too long.
   * @deprecated Minecraft no longer uses textures packs. Instead you should use {@link
   ...
   * <p>
   * The player's client will download the new resource pack asynchronously in the background, and
   * will automatically switch to it once the download is complete. If the client has downloaded and
   * cached the same resource pack in the past, it will perform a quick timestamp check over the
   * network to determine if the resource pack has changed and needs to be downloaded again. When
   * this request is sent for the very first time from a given server, the client will first display
   * a confirmation GUI to the player before proceeding with the download.
   * <p>
   * Notes:
   ...
   * </ul>
   *
   * @param url The URL from which the client will download the resource pack. The string must
   * contain only US-ASCII characters and should be encoded as per RFC 1738.
   * @throws IllegalArgumentException Thrown if the URL is null.
   * @throws IllegalArgumentException Thrown if the URL is too long. The length restriction is an
   ...
   * @throws IllegalArgumentException if scoreboard was not created by the {@link
   * org.bukkit.scoreboard.ScoreboardManager scoreboard manager}
   * @throws IllegalStateException if this is a player that is not logged yet or has logged out
   */
  void setScoreboard(Scoreboard scoreboard) throws IllegalArgumentException, IllegalStateException;

  /**
   * Gets if the client is displayed a 'scaled' health, that is, health on a scale from 0-{@link
   * #getHealthScale()}.
   *
  ...
  /**
   * Sets if the client is displayed a 'scaled' health, that is, health on a scale from 0-{@link
   * #getHealthScale()}.
   * <p>
   * Displayed health follows a simple formula <code>displayedHealth = getHealth() / getMaxHealth()
   * * getHealthScale()</code>.
   *
  ...
  /**
   * Sets the number to scale health to for the client; this will also {@link
   ...
   *
   * @return the number that health would be scaled to for the client if HealthScaling is set to
   * true
   ...
   *
   * @return the followed entity, or null if not in spectator mode or not following a specific
   * entity.
   */
   ...
   * @throws IllegalStateException if the player is not in {@link GameMode#SPECTATOR}
   */
   ...
   * will not be sent and the display will remain unchanged. If they are empty strings, the display
   * will be updated as such. If the strings contain a new line, only the first line will be sent.
   *
  ...
  class Spigot extends Entity.Spigot {
    ...
    /**
     * Gets the connection address of this player, regardless of whether it has been spoofed or
     * not.
     *
