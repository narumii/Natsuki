...
public interface LivingEntity extends Entity, Damageable, ProjectileSource {
   ...
   * @param maxDistance this is the maximum distance to scan (may be limited by server by at least
   * 100 blocks, no less)
   ...
   * This list contains all blocks from the living entity's eye position to target inclusive.
   *
   * @param transparent HashSet containing all transparent block Materials (set to null for only
   * air)
   * @param maxDistance this is the maximum distance to scan (may be limited by server by at least
   * 100 blocks, no less)
   * @return list containing all blocks along the living entity's line of sight
   ...
   *
   * @param transparent HashSet containing all transparent block IDs (set to null for only air)
   * @param maxDistance this is the maximum distance to scan. This may be further limited by the
   * server, but never to less than 100 blocks
  ...
  /**
   * Returns whether the living entity already has an existing effect of the given {@link
   ...
   * <p>
   * This method has no effect on EnderDragons, Withers, Players, or Bats. Non-living entities
