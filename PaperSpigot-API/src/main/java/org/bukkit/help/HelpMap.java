...
/**
 * The HelpMap tracks all help topics registered in a Bukkit server. When the server starts up or is
 * reloaded, help is processed and topics are added in the following order:
 *
 * <ol>
...
public interface HelpMap {
   ...
   * call this method during {@code onLoad()}. Once registered, the custom HelpTopicFactory will be
   * used to create a custom {@link HelpTopic} for all commands deriving from the {@code
   * commandClass} base class, or all commands deriving from {@link org.bukkit.command.PluginCommand}
   * who's executor derives from {@code commandClass} base class.
   *
   ...
   * @param factory The {@link HelpTopicFactory} implementation to associate with the {@code
   * commandClass}.
   * @throws IllegalArgumentException Thrown if {@code commandClass} does not derive from a legal
   * base class.
   */
   ...
   * Plugin authors who choose to directly extend {@link org.bukkit.command.Command} instead of
   * {@link org.bukkit.command.PluginCommand} will need to check this collection in their {@link
   * HelpTopicFactory} implementations to ensure they meet the server administrator's expectations.
   *
