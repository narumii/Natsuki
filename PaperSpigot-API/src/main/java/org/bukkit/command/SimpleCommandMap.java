...
public class SimpleCommandMap implements CommandMap {
  ...
  public boolean register(String label, String fallbackPrefix, Command command) {
    command.timings = co.aikar.timings.TimingsManager
        .getCommandTiming(fallbackPrefix, command); // Spigot
  ...
  public List<String> tabComplete(CommandSender sender, String cmdLine) {
    return tabComplete(sender, cmdLine,
        null); // PaperSpigot - location tab-completes, code moved below
  }
  ...
  public List<String> tabComplete(CommandSender sender, String cmdLine, Location location) {
    ...
    try {
      return target
          .tabComplete(sender, commandName, args, location); // PaperSpigot - add location argument
    } catch (CommandException ex) {
      throw ex;
    } catch (Throwable ex) {
      ...
      server.getPluginManager().callEvent(new ServerExceptionEvent(
          new ServerTabCompleteException(msg, ex, target, sender, args))); // Paper
      throw new CommandException(msg, ex);
    }
  }
  // PaperSpigot end
  ...
  public void registerServerAliases() {
    ...
    for (String alias : values.keySet()) {
      ...
      if (targets.size() > 0) {
        knownCommands.put(alias.toLowerCase(), new FormattedCommandAlias(alias.toLowerCase(),
