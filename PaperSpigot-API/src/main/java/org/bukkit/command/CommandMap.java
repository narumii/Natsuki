...
public interface CommandMap {
   ...
   * </ul>
   *
   * @param fallbackPrefix a prefix which is prepended to each command with a ':' one or more times
  ...
  /**
   * Registers a command. Returns true on success; false if name is already taken and fallback had
   * to be used.
   * <p>
   * Caller can use:-
   * <ul>
   ...
   * @return true if command was registered with the passed in label, false otherwise, which
   * indicates the fallbackPrefix was used one or more times
   */
  ...
  /**
   * Looks for the requested command and executes an appropriate tab-completer if found. This method
   * will also tab-complete partial commands.
   *
   * @param sender The command's sender.
   * @param cmdLine The entire command string to tab-complete, excluding initial slash.
   * @return a list of possible tab-completions. This list may be immutable. Will be null if no
   * matching command of which sender has permission.
   * @throws CommandException Thrown when the tab-completer for the given command fails with an
   * unhandled exception
