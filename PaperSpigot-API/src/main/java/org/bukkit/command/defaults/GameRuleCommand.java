...
public class GameRuleCommand extends VanillaCommand {
    ...
    if (args.length > 0) {
      ...
      if (world.isGameRule(rule)) {
        if (args.length > 1) {
          ...
          Command
              .broadcastCommandMessage(sender, "Game rule " + rule + " has been set to: " + value);
        } else {
      ...
      sender
          .sendMessage("Rules: " + this.createString(getGameWorld(sender).getGameRules(), 0, ", "));
  ...
  public List<String> tabComplete(CommandSender sender, String alias, String[] args)
    ...
    if (args.length == 1) {
      return StringUtil
          .copyPartialMatches(args[0], Arrays.asList(getGameWorld(sender).getGameRules()),
