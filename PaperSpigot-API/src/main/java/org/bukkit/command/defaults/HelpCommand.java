...
public class HelpCommand extends VanillaCommand {
  ...
  public boolean execute(CommandSender sender, String currentAlias, String[] args) {
    ...
    ChatPaginator.ChatPage page = ChatPaginator
        .paginate(topic.getFullText(sender), pageNumber, pageWidth, pageHeight);
  ...
  public List<String> tabComplete(CommandSender sender, String alias, String[] args) {
    ...
    if (args.length == 1) {
      ...
      for (HelpTopic topic : Bukkit.getServer().getHelpMap().getHelpTopics()) {
        String trimmedTopic =
            topic.getName().startsWith("/") ? topic.getName().substring(1) : topic.getName();
  ...
  protected HelpTopic findPossibleMatches(String searchString) {
    ...
    for (HelpTopic topic : Bukkit.getServer().getHelpMap().getHelpTopics()) {
      ...
      if (Character.toLowerCase(trimmedTopic.charAt(0)) != Character
          .toLowerCase(searchString.charAt(0))) {
        continue;
      }
    ...
    if (possibleMatches.size() > 0) {
      return new IndexHelpTopic("Search", null, null, possibleMatches,
   ...
   * <a href="http://en.wikipedia.org/wiki/Damerau%E2%80%93Levenshtein_distance">Wikipedia:
   * Damerau–Levenshtein distance</a>
   *
   * @param s1 The first string being compared.
   * @param s2 The second string being compared.
   * @return The number of substitutions, deletions, insertions, and transpositions required to get
   * from s1 to s2.
   */
