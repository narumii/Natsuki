...
public class PluginsCommand extends BukkitCommand {
  ...
  private TextComponent[] getPluginList() {
    ...
    for (final Plugin plugin : plugins) {
      ...
      textComponents[offset] = new TextComponent(
          color + plugin.getDescription().getName() + ChatColor.WHITE + ", ");
      textComponents[offset].setHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT,
          new ComponentBuilder(ChatColor.GRAY + plugin.getDescription().getDescription())
              .create()));
      textComponents[offset].setClickEvent(new ClickEvent(ClickEvent.Action.RUN_COMMAND,
          "/pl toggle " + plugin.getDescription().getName()));
      offset++;
    }
    if (plugins.length != 0) {
      textComponents[plugins.length]
          .setText(textComponents[plugins.length].getText().replace(",", ""));
      textComponents[0] = new TextComponent(
          " §8» §7Plugins: §8(§a" + enabled + "§8/§c" + disbaled + "§8/§7" + plugins.length
              + "§8)§7: ");
    }
    if (plugins.length == 0) {
  ...
  private String getConsolePluginList() {
    ...
    if (plugins.length == 0) {
      return " §8» §7Server doesn't contains any plugins.";
    } else {
      return " §8» §7Plugins: §8(§a" + enabled + "§8/§c" + disbaled + "§8/§7" + plugins.length
          + "§8)§7: " + stringBuilder.toString();
    }
  }
