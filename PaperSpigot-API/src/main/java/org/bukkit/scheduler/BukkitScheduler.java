...
public interface BukkitScheduler {
   ...
   * <p>
   * Schedules a once off task to occur after a delay. This task will be executed by a thread
   ...
   * @param delay Delay in server ticks before executing task
   * @return Task id number (-1 if scheduling failed)
   * @deprecated This name is misleading, as it does not schedule "a sync" task, but rather, "an
   * async" task
   */
  @Deprecated
   ...
   * should be taken to assure the thread-safety of asynchronous tasks.</b>
   * <p>
   * Schedules a once off task to occur as soon as possible. This task will be executed by a thread
   * managed by the scheduler.
   *
   * @param plugin Plugin that owns the task
   * @param task Task to be executed
   ...
   * <p>
   * A repeating task might not be running currently, but will be running in the future. A task that
   * has finished, and does not repeat, will not be running ever again.
   * <p>
   ...
   * <p>
   * If a repeating task is currently running, it might not be queued now but could be in the
  ...
  /**
   * Returns a list of all pending tasks. The ordering of the tasks is not related to their order of
   * execution.
   *
   * @return Active workers
   */
   ...
   */
  @Deprecated
  BukkitTask runTaskAsynchronously(Plugin plugin, BukkitRunnable task)
  ...
  /**
   * Returns a task that will repeatedly run until cancelled, starting after the specified number of
   * server ticks.
   *
   * @param plugin the reference to the plugin scheduling task
   * @param task the task to be run
