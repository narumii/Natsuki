...
public class ShapedRecipe implements Recipe {
   ...
   * corresponds with the upper most part of the recipe on the workbench e.g. if all three rows are
   * supplies the first string represents the top row on the workbench.
   *
  ...
  public ShapedRecipe shape(final String... shape) {
    ...
    Validate.isTrue(shape.length > 0 && shape.length < 4,
        "Crafting recipes should be 1, 2, 3 rows, not ", shape.length);

    for (String row : shape) {
      Validate.notNull(row, "Shape cannot have null rows");
      Validate.isTrue(row.length() > 0 && row.length() < 4,
