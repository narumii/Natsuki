...
/**
 * Represents a view linking two inventories and a single player (whose inventory may or may not be
 * one of the two).
 * <p>
 * Note: If you implement this interface but fail to satisfy the expected contracts of certain
 * methods, there's no guarantee that the game will work as it should.
 */
public abstract class InventoryView {
  ...
  /**
   * Determine the type of inventory involved in the transaction. This indicates the window style
   * being shown. It will never return PLAYER, since that is common to all windows.
   *
   ...
   * <p>
   * Note: If slot ID -999 is chosen, it may be expected that the item is dropped on the ground.
   * This is not required behaviour, however.
   *
  ...
  /**
   * Converts a raw slot ID into its local slot ID into whichever of the two inventories the slot
   * points to.
   * <p>
   * If the raw slot refers to the upper inventory, it will be returned unchanged and thus be
   * suitable for getTopInventory().getItem(); if it refers to the lower inventory, the output will
   * differ from the input and be suitable for getBottomInventory().getItem().
   *
   ...
   * <p>
   * Note though that it's possible for this to be greater than the sum of the two inventories if
   * for example some slots are not being used.
   *
  ...
  /**
   * Sets an extra property of this inventory if supported by that inventory, for example the state
   * of a progress bar.
   *
   * @param prop the window property to update
   * @param value the new value for the window property
   * @return true if the property was updated successfully, false if the property is not supported
   * by that inventory
   */
