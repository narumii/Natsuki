...
public class WorldCreator {
   ...
   * This may be null, in which case the "natural" generator for this environment will be used.
   * <p>
   * If the generator cannot be found for the given name, the natural environment generator will be
   * used instead and a warning will be printed to the console.
   *
   ...
   * <p>
   * If the generator cannot be found for the given name, the natural environment generator will be
   * used instead and a warning will be printed to the specified output
   *
   * @param generator Name of the generator to use, in "plugin:id" notation
   ...
   * <p>
   * If the generator is not found, null will be returned and a message will be printed to the
   * specified {@link CommandSender} explaining why.
   * <p>
   * The name must be in the "plugin:id" notation, or optionally just "plugin", where "plugin" is
   * the safe-name of a plugin and "id" is an optional unique identifier for the generator you wish
   * to request from the plugin.
   *
   ...
   */
  public static ChunkGenerator getGeneratorForName(String world, String name,
    ...
    if (name != null) {
      ...
      if (plugin == null) {
        output.sendMessage("Could not set generator for world '" + world + "': Plugin '" + split[0]
            + "' does not exist");
      } else if (!plugin.isEnabled()) {
        output.sendMessage(
            "Could not set generator for world '" + world + "': Plugin '" + plugin.getDescription()
                .getFullName() + "' is not enabled");
      } else {
