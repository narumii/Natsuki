...
/**
 * A ConversationFactory is responsible for creating a {@link Conversation} from a predefined
 * template. A ConversationFactory is typically created when a plugin is instantiated and builds a
 * Conversation each time a user initiates a conversation with the plugin. Each Conversation
 * maintains its own state and calls back as needed into the plugin.
 * <p>
 * The ConversationFactory implements a fluid API, allowing parameters to be set as an extension to
 * the constructor.
 */
public class ConversationFactory {
  ...
  /**
   * Sets the modality of all {@link Conversation}s created by this factory. If a conversation is
   * modal, all messages directed to the player are suppressed for the duration of the
   * conversation.
   * <p>
   * The default is True.
   *
  ...
  /**
   * Sets the local echo status for all {@link Conversation}s created by this factory. If local echo
   * is enabled, any text submitted to a conversation gets echoed back into the submitter's chat
   * window.
   *
  ...
  /**
   * Sets the number of inactive seconds to wait before automatically abandoning all generated
   * conversations.
   * <p>
   ...
   *
   * @param playerOnlyMessage The message to return to a non-play in lieu of starting a
   * conversation.
   * @return This object.
   */
   ...
   * @return This object.
   */
  public ConversationFactory addConversationAbandonedListener(
  ...
  public Conversation buildConversation(Conversable forWhom) {
    ...
    Conversation conversation = new Conversation(plugin, forWhom, firstPrompt,
