...
/**
 * The Conversation class is responsible for tracking the current state of a conversation,
 * displaying prompts to the user, and dispatching the user's response to the appropriate place.
 * Conversation objects are not typically instantiated directly. Instead a {@link
 * ConversationFactory} is used to construct identical conversations on demand.
 * <p>
 * Conversation flow consists of a directed graph of {@link Prompt} objects. Each time a prompt gets
 * input from the user, it must return the next prompt in the graph. Since each Prompt chooses the
 * next Prompt, complex conversation trees can be implemented where the nature of the player's
 * response directs the flow of the conversation.
 * <p>
 * Each conversation has a {@link ConversationPrefix} that prepends all output from the conversation
 * to the player. The ConversationPrefix can be used to display the plugin name or conversation
 * status as the conversation evolves.
 * <p>
 * Each conversation has a timeout measured in the number of inactive seconds to wait before
 * abandoning the conversation. If the inactivity timeout is reached, the conversation is abandoned
 * and the user's incoming and outgoing chat is returned to normal.
 * <p>
 * You should not construct a conversation manually. Instead, use the {@link ConversationFactory}
...
public class Conversation {
   ...
   * @param forWhom The entity for whom this conversation is mediating.
   * @param firstPrompt The first prompt in the conversation graph.
   * @param initialSessionData Any initial values to put in the conversation context sessionData
   * map.
   */
  public Conversation(Plugin plugin, Conversable forWhom, Prompt firstPrompt,
  ...
  /**
   * Gets the modality of this conversation. If a conversation is modal, all messages directed to
   * the player are suppressed for the duration of the conversation.
   *
  ...
  /**
   * Sets the modality of this conversation.  If a conversation is modal, all messages directed to
   * the player are suppressed for the duration of the conversation.
   *
  ...
  /**
   * Gets the status of local echo for this conversation. If local echo is enabled, any text
   * submitted to a conversation gets echoed back into the submitter's chat window.
   *
  ...
  /**
   * Sets the status of local echo for this conversation. If local echo is enabled, any text
   * submitted to a conversation gets echoed back into the submitter's chat window.
   *
  ...
  /**
   * Displays the first prompt of this conversation and begins redirecting the user's chat
   * responses.
   */
  public void begin() {
  ...
  public void acceptInput(String input) {
    try { // Spigot
      ...
      org.bukkit.Bukkit.getLogger()
          .log(java.util.logging.Level.SEVERE, "Error handling conversation prompt", t);
    }
    // Spigot End
  }
   ...
   */
  public synchronized void addConversationAbandonedListener(
   ...
   */
  public synchronized void removeConversationAbandonedListener(
      ConversationAbandonedListener listener) {
