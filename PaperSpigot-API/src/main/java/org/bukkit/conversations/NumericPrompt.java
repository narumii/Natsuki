...
/**
 * NumericPrompt is the base class for any prompt that requires a {@link Number} response from the
 * user.
 */
public abstract class NumericPrompt extends ValidatingPrompt {
  ...
  /**
   * Override this method to do further validation on the numeric player input after the input has
   * been determined to actually be a number.
   *
  ...
  /**
   * Optionally override this method to display an additional message if the user enters an invalid
   * number.
   *
   * @param context Context information about the conversation.
