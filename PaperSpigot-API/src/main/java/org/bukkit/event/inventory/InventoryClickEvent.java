 ...
 * <p>
 * Because InventoryClickEvent occurs within a modification of the Inventory, not all Inventory
 * related methods are safe to use.
 * <p>
 * The following should never be invoked by an EventHandler for InventoryClickEvent using the
 * HumanEntity or InventoryView associated with this event:
 * <ul>
 ...
 * </ul>
 * To invoke one of these methods, schedule a task using {@link BukkitScheduler#runTask(Plugin,
 * Runnable)}, which will run the task on the next tick. Also be aware that this is not an
 * exhaustive list, and other methods could potentially create issues as well.
 * <p>
 * Assuming the EntityHuman associated with this event is an instance of a Player, manipulating the
 * MaxStackSize or contents of an Inventory will require an Invocation of {@link
 * Player#updateInventory()}.
 * <p>
 * Modifications to slots that are modified by the results of this InventoryClickEvent can be
 * overwritten. To change these slots, this event should be cancelled and all desired changes to the
 * inventory applied. Alternatively, scheduling a task using {@link BukkitScheduler#runTask(Plugin,
 * Runnable)}, which would execute the task on the next tick, would work as well.
 */
public class InventoryClickEvent extends InventoryInteractEvent {
  ...
  @Deprecated
  public InventoryClickEvent(InventoryView view, SlotType type, int slot, boolean right,
      boolean shift) {
  ...
  public InventoryClickEvent(InventoryView view, SlotType type, int slot, ClickType click,
  ...
  /**
   * Gets whether the ClickType for this event indicates that the key was pressed down when the
   * click was made.
   *
   ...
   * @deprecated This changes the ItemStack in their hand before any calculations are applied to the
   * Inventory, which has a tendency to create inconsistencies between the Player and the server,
   * and to make unexpected changes in the behavior of the clicked Inventory.
   */
  @Deprecated
   ...
   * that there may be two slots with the same slot number, since a view links two different
   * inventories.
   *
  ...
  /**
   * The raw slot number clicked, ready for passing to {@link InventoryView #getItem(int)} This slot
   * number is unique for the view.
   *
   * @return the slot number
   */
   ...
   * change the behavior of this InventoryClickEvent, changes must be manually applied.
   *
