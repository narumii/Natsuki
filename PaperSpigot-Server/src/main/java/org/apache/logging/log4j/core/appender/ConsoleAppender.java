 ...
 *
 * @doubt accessing System.out or .err as a byte stream instead of a writer bypasses the JVM's
 * knowledge of the proper encoding. (RG) Encoding is handled within the Layout. Typically, a Layout
 * will generate a String and then call getBytes which may use a configured encoding or the system
...
public final class ConsoleAppender extends AbstractOutputStreamAppender {
  ...
  private ConsoleAppender(final String name, final Layout<? extends Serializable> layout,
      final Filter filter,
  ...
  public static ConsoleAppender createAppender(
    ...
    return new ConsoleAppender(name, layout, filter, getManager(isFollow, target, layout),
        ignoreExceptions);
  }
  ...
  private static OutputStreamManager getManager(final boolean follow, final Target target,
    ...
    return OutputStreamManager
        .getManager(target.name() + "." + follow, new FactoryData(os, type, layout), factory);
  }
