...
public class AntiXray {
  ...
  /**
   * Starts the timings handler, then updates all blocks within the set radius of the given
   * coordinate, revealing them if they are hidden ores.
   */
  public void updateNearbyBlocks(World world, BlockPosition position) {
    if (world.spigotConfig.antiXray) {
      ...
      updateNearbyBlocks(world, position, 2,
          false); // 2 is the radius, we shouldn't change it as that would make it exponentially slower
      SpigotTimings.antiXrayUpdateTimer.stopTiming();
    }
  }

  /**
   * Starts the timings handler, and then removes all non exposed ores from the chunk buffer.
   */
  ...
  private void updateNearbyBlocks(World world, BlockPosition position, int radius,
      boolean updateSelf) {
  ...
  private static boolean hasTransparentBlockAdjacent(World world, BlockPosition position,
      int radius) {
