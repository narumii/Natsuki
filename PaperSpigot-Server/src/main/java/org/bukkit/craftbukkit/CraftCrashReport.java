  ...
  public Object call() throws Exception {
    ...
    try {
      ...
      for (Plugin plugin : Bukkit.getPluginManager().getPlugins()) {
        ...
        value.append(' ').append(description.getFullName()).append(' ')
      ...
      value.append("\n   Reload Count: ")
          .append(String.valueOf(MinecraftServer.getServer().server.reloadCount));
      value.append("\n   Threads: {");
      for (Map.Entry<Thread, ? extends Object[]> entry : Thread.getAllStackTraces().entrySet()) {
        value.append(' ').append(entry.getKey().getState().name()).append(' ')
            .append(entry.getKey().getName()).append(": ").append(Arrays.toString(entry.getValue()))
            .append(',');
      }
