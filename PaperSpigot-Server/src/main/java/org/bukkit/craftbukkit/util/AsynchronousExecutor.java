...
/**
 * Executes tasks using a multi-stage process executor. Synchronous executions are via {@link
 * AsynchronousExecutor#finishActive()} or the {@link AsynchronousExecutor#get(Object)} methods. <li
 * \> Stage 1 creates the object from a parameter, and is usually called asynchronously. <li \>
 * Stage 2 takes the parameter and object from stage 1 and does any synchronous processing to
 * prepare it. <li \> Stage 3 takes the parameter and object from stage 1, as well as a callback
 * that was registered, and performs any synchronous calculations.
 *
 * @param <P> The type of parameter you provide to make the object that will be created. It should
 * implement {@link Object#hashCode()} and {@link Object#equals(Object)} if you want to get the
 * value early.
 * @param <T> The type of object you provide. This is created in stage 1, and passed to stage 2, 3,
 * and returned if get() is called.
 * @param <C> The type of callback you provide. You may register many of these to be passed to the
 * provider in stage 3, one at a time.
...
public final class AsynchronousExecutor<P, T, C, E extends Throwable> {
  ...
  class Task implements Runnable {
    ...
    void initSync() {
      if (set(this, PENDING, STAGE_1_COMPLETE)) {
        ...
        synchronized (this) {
          if (set(this, STAGE_1_SYNC,
              PENDING)) { // They might NOT synchronized yet, atomic lock IS needed
   ...
   * <p>
   * If no callbacks remain for a given parameter, then the {@link CallBackProvider
   * CallBackProvider's} stages may be omitted from execution. Stage 3 will have no callbacks, stage
   * 2 will be skipped unless a {@link #get(Object)} is used, and stage 1 will be avoided on a
   * best-effort basis.
   * <p>
   ...
   * This should always be synchronous
   *
   * @return true if no further execution for the parameter is possible, such that, no exceptions
   * will be thrown in {@link #finishActive()} for the parameter, and {@link #get(Object)} will
   * throw an {@link IllegalStateException}, false otherwise
   ...
   * This should always be synchronous.
   *
   * @throws IllegalStateException if the parameter is not in the queue anymore, or sometimes if
