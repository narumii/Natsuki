...
public final class CraftProfileBanEntry implements org.bukkit.BanEntry {
  ...
  public CraftProfileBanEntry(GameProfile profile, GameProfileBanEntry entry,
  ...
  @Override
  public void save() {
    GameProfileBanEntry entry = new GameProfileBanEntry(profile, this.created, this.source,
        this.expiration, this.reason);
    this.list.add(entry);
    try {
      this.list.save();
    } catch (IOException ex) {
      Bukkit.getLogger()
          .log(Level.SEVERE, "Failed to save banned-players.json, {0}", ex.getMessage());
    }
  }
}
