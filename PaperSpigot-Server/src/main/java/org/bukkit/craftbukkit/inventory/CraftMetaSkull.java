...
class CraftMetaSkull extends CraftMetaItem implements SkullMeta {
  ...
  CraftMetaSkull(NBTTagCompound tag) {
    ...
    if (Natsuki.getInstance().getConfig().PACKET.NBT.fixSkull) {
      ...
      } else if (tag.hasKeyOfType(SKULL_OWNER.NBT, 8) && isValid(
          tag.getCompound(SKULL_OWNER.NBT))) {
        profile = new GameProfile(null, tag.getString(SKULL_OWNER.NBT));
      }
    } else {
      if (tag.hasKeyOfType(SKULL_OWNER.NBT, 10)) {
        profile = GameProfileSerializer.deserialize(tag.getCompound(SKULL_OWNER.NBT));
      } else if (tag.hasKeyOfType(SKULL_OWNER.NBT, 8) && !tag.getString(SKULL_OWNER.NBT)
          .isEmpty()) {
        profile = new GameProfile(null, tag.getString(SKULL_OWNER.NBT));
      }
    }
  ...
  private boolean isValid(final NBTTagCompound tagCompound) {
    final String name = tagCompound.hasKey("Name") ? tagCompound.getString("Name") : "";
    final String id = tagCompound.hasKey("Id") ? tagCompound.getString("Id") : "";

    return name.length() >= 3 && name.length() <= 16 && id.length() >= 8 && id.length() <= 64
        && !tagCompound.isEmpty();
  }
  ...
  void applyToItem(final NBTTagCompound tag) { // Spigot - make final
    ...
    if (profile != null) {
      ...
      net.minecraft.server.TileEntitySkull
          .b(profile, new com.google.common.base.Predicate<GameProfile>() {
  ...
  boolean equalsCommon(CraftMetaItem meta) {
    ...
    if (meta instanceof CraftMetaSkull) {
      ...
      return (this.hasOwner() ? that.hasOwner() && this.profile.equals(that.profile)
          : !that.hasOwner());
    }
    return true;
  }
