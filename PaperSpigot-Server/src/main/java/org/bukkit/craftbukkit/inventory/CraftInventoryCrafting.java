...
public class CraftInventoryCrafting extends CraftInventory implements CraftingInventory {
  ...
  public CraftItemStack getItem(int index) {
    if (index < getResultInventory().getSize()) {
      ...
      net.minecraft.server.ItemStack item = getMatrixInventory()
          .getItem(index - getResultInventory().getSize());
      return item == null ? null : CraftItemStack.asCraftMirror(item);
    }
  }
  ...
  public void setMatrix(ItemStack[] contents) {
    if (getMatrixInventory().getContents().length > contents.length) {
      throw new IllegalArgumentException(
          "Invalid inventory size; expected " + getMatrixInventory().getContents().length
              + " or less");
    }
