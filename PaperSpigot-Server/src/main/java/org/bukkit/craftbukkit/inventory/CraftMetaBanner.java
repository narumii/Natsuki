...
public class CraftMetaBanner extends CraftMetaItem implements BannerMeta {
  ...
  CraftMetaBanner(NBTTagCompound tag) {
    ...
    base = entityTag.hasKey(BASE.NBT) ? DyeColor.getByDyeData((byte) entityTag.getInt(BASE.NBT))
        : null;
  ...
  CraftMetaBanner(Map<String, Object> map) {
    ...
    Iterable<?> rawPatternList = SerializableMeta
        .getObject(Iterable.class, map, PATTERNS.BUKKIT, true);
    ...
    for (Object obj : rawPatternList) {
      if (!(obj instanceof Pattern)) {
        throw new IllegalArgumentException(
            "Object in pattern list is not valid. " + obj.getClass());
      }
