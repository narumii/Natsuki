...
public final class VanillaCommandWrapper extends VanillaCommand {
  ...
  public List<String> tabComplete(CommandSender sender, String alias, String[] args)
    ...
    return tabComplete(sender, alias, args,
        null); // PaperSpigot - location tab-completes. Original code moved below
  }
  ...
  public final int dispatchVanillaCommand(CommandSender bSender, ICommandListener icommandlistener,
      String[] as) {
    ...
    try {
      if (vanillaCommand.canUse(icommandlistener)) {
        if (i > -1) {
          ...
          icommandlistener
              .a(CommandObjectiveExecutor.EnumCommandResult.AFFECTED_ENTITIES, list.size());
          ...
          while (iterator.hasNext()) {
            ...
            try {
              ...
              ChatMessage chatmessage = new ChatMessage("commands.generic.usage",
      ...
      if (icommandlistener.f() instanceof EntityMinecartCommandBlock) {
        MinecraftServer.LOGGER.log(Level.WARN, String
            .format("MinecartCommandBlock at (%d,%d,%d) failed to handle command",
                ...
                icommandlistener.getChunkCoordinates().getZ()), throwable);
        ...
        MinecraftServer.LOGGER.log(Level.WARN, String
            .format("CommandBlock at (%d,%d,%d) failed to handle command",
                ...
                listener.getChunkCoordinates().getZ()), throwable);
      } else {
        MinecraftServer.LOGGER
            .log(Level.WARN, String.format("Unknown CommandBlock failed to handle command"),
                throwable);
      }
    } finally {
  ...
  private ICommandListener getListener(CommandSender sender) {
    ...
    if (sender instanceof CommandMinecart) {
      return ((EntityMinecartCommandBlock) ((CraftMinecartCommand) sender).getHandle())
          .getCommandBlock();
    }
