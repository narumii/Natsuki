...
public class EntityEnderDragon extends EntityInsentient implements IComplex, IMonster {
  ...
  private Explosion explosionSource = new Explosion(null, this, Double.NaN, Double.NaN, Double.NaN,
  ...
  public void m() {
    ...
    if (this.getHealth() <= 0.0F) {
      ...
      this.world.addParticle(EnumParticle.EXPLOSION_LARGE, this.locX + (double) f,
          this.locY + 2.0D + (double) f1, this.locZ + (double) f2, 0.0D, 0.0D, 0.0D);
    } else {
      this.n();
      f = (float) (0.2F / (Math.sqrt(this.motX * this.motX + this.motZ * this.motZ) * 10.0F
          + 1.0F));
      ...
      if (this.ce()) {
        ...
        if (this.world.isClientSide) {
          ...
          this.bb *= 0.8F;
          float f5 = (float) (Math.sqrt(this.motX * this.motX + this.motZ * this.motZ) * 1.0F
              + 1.0F);
  ...
  private void cf() {
    ...
    if (this.random.nextInt(2) == 0 && !arraylist.isEmpty()) {
      ...
      EntityTargetEvent event = new EntityTargetEvent(this.getBukkitEntity(),
          target.getBukkitEntity(), EntityTargetEvent.TargetReason.RANDOM_TARGET);
      this.world.getServer().getPluginManager().callEvent(event);
  ...
  private boolean b(AxisAlignedBB axisalignedbb) {
        ...
        for (int i2 = k; i2 <= j1; ++i2) {
          ...
          if (block.getMaterial() != Material.AIR) {
            if (block != Blocks.BARRIER && block != Blocks.OBSIDIAN && block != Blocks.END_STONE
                && block != Blocks.BEDROCK && block != Blocks.COMMAND_BLOCK && this.world
    ...
    if (flag1) {
      ...
      EntityExplodeEvent event = new EntityExplodeEvent(bukkitEntity, bukkitEntity.getLocation(),
          destroyedBlocks, 0F);
      ...
      // CraftBukkit end
      double d0 =
          axisalignedbb.a + (axisalignedbb.d - axisalignedbb.a) * (double) this.random.nextFloat();
      double d1 =
          axisalignedbb.b + (axisalignedbb.e - axisalignedbb.b) * (double) this.random.nextFloat();
      double d2 =
          axisalignedbb.c + (axisalignedbb.f - axisalignedbb.c) * (double) this.random.nextFloat();
  ...
  protected void aZ() {
    ...
    if (this.by >= 180 && this.by <= 200) {
      ...
      this.world.addParticle(EnumParticle.EXPLOSION_HUGE, this.locX + (double) f,
          this.locY + 2.0D + (double) f1, this.locZ + (double) f2, 0.0D, 0.0D, 0.0D);
    }
    ...
    if (!this.world.isClientSide) {
      if (this.by > 150 && this.by % 5 == 0 && flag) {
        i = this.expToDrop
            / 12; // CraftBukkit - drop experience as dragon falls from sky. use experience drop from death event. This is now set in getExpReward()
      ...
      if (this.by == 1) {
        ...
        for (EntityPlayer player : MinecraftServer.getServer().getPlayerList().players) {
          ...
          if (world.spigotConfig.dragonDeathSoundRadius > 0 && distanceSquared
    ...
    if (this.by == 200 && !this.world.isClientSide) {
      if (flag) {
        i = this.expToDrop - (10 * this.expToDrop
            / 12); // CraftBukkit - drop the remaining experience
