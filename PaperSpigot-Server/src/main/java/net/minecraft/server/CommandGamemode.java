...
public class CommandGamemode extends CommandAbstract {
  ...
  public void execute(ICommandListener icommandlistener, String[] astring) throws CommandException {
    if (astring.length <= 0) {
      ...
      EntityPlayer entityplayer =
          astring.length >= 2 ? a(icommandlistener, astring[1]) : b(icommandlistener);
      ...
      if (entityplayer.playerInteractManager.getGameMode() != worldsettings_enumgamemode) {
        icommandlistener.sendMessage(new ChatComponentText(
            "Failed to set the gamemode of '" + entityplayer.getName() + "'"));
        return;
      }
      // CraftBukkit end
      ...
      if (entityplayer != icommandlistener) {
        a(icommandlistener, this, 1, "commands.gamemode.success.other", entityplayer.getName(),
            chatmessage);
      } else {
  ...
  protected WorldSettings.EnumGamemode h(ICommandListener icommandlistener, String s)
    ...
    return !s.equalsIgnoreCase(WorldSettings.EnumGamemode.SURVIVAL.b()) && !s.equalsIgnoreCase("s")
        ...
        ? (
        !s.equalsIgnoreCase(WorldSettings.EnumGamemode.SPECTATOR.b()) && !s.equalsIgnoreCase("sp")
            ? WorldSettings.a(a(s, 0, WorldSettings.EnumGamemode.values().length - 2))
            : WorldSettings.EnumGamemode.SPECTATOR) : WorldSettings.EnumGamemode.ADVENTURE)
        : WorldSettings.EnumGamemode.CREATIVE) : WorldSettings.EnumGamemode.SURVIVAL;
  }
