...
public class BlockSnow extends Block {
  ...
  public AxisAlignedBB a(World world, BlockPosition blockposition, IBlockData iblockdata) {
    ...
    return new AxisAlignedBB((double) blockposition.getX() + this.minX,
        ...
        (double) blockposition.getX() + this.maxX,
        (double) ((float) blockposition.getY() + (float) i * f),
  ...
  public boolean canPlace(World world, BlockPosition blockposition) {
    ...
    return (block != Blocks.ICE && block != Blocks.PACKED_ICE) && (
        block.getMaterial() == Material.LEAVES || (
            block == this && iblockdata.get(BlockSnow.LAYERS).intValue() >= 7
  ...
  public void b(World world, BlockPosition blockposition, IBlockData iblockdata, Random random) {
    if (world.b(EnumSkyBlock.BLOCK, blockposition) > 11) {
      // CraftBukkit start
      if (org.bukkit.craftbukkit.event.CraftEventFactory.callBlockFadeEvent(world.getWorld()
          .getBlockAt(blockposition.getX(), blockposition.getY(), blockposition.getZ()), Blocks.AIR)
          .isCancelled()) {
        return;
      }
      // CraftBukkit end
