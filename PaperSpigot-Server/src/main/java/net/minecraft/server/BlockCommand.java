...
public class BlockCommand extends BlockContainer {
  ...
  public void doPhysics(World world, BlockPosition blockposition, IBlockData iblockdata,
    ...
    if (!world.isClientSide) {
      ...
      if (eventRedstone.getNewCurrent() > 0 && !(eventRedstone.getOldCurrent()
          > 0)) { // CraftBukkit
        world.setTypeAndData(blockposition,
            iblockdata.set(BlockCommand.TRIGGERED, Boolean.valueOf(true)), 4);
        world.a(blockposition, this, this.a(world));
      } else if (!(eventRedstone.getNewCurrent() > 0)
          && eventRedstone.getOldCurrent() > 0) { // CraftBukkit
        world.setTypeAndData(blockposition,
            iblockdata.set(BlockCommand.TRIGGERED, Boolean.valueOf(false)), 4);
      }
    }
  ...
  public boolean interact(World world, BlockPosition blockposition, IBlockData iblockdata,
    ...
    return tileentity instanceof TileEntityCommand && ((TileEntityCommand) tileentity)
        .getCommandBlock().a(entityhuman);
  }
