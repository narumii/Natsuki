...
public class WorldMap extends PersistentBase {
  ...
  public void a(EntityHuman entityhuman, ItemStack itemstack) {
      ...
      if (!worldmap_worldmaphumantracker1.trackee.dead && (
        ...
        if (!itemstack.y() && worldmap_worldmaphumantracker1.trackee.dimension == this.map) {
          this.a(0, worldmap_worldmaphumantracker1.trackee.world,
              ...
              worldmap_worldmaphumantracker1.trackee.locX,
              worldmap_worldmaphumantracker1.trackee.locZ,
    ...
    if (itemstack.hasTag() && itemstack.getTag().hasKeyOfType("Decorations", 9)) {
      ...
      for (int j = 0; j < nbttaglist.size(); ++j) {
        ...
        // Spigot - start
        UUID uuid = UUID
            .nameUUIDFromBytes(nbttagcompound.getString("id").getBytes(Charsets.US_ASCII));
        if (!this.decorations.containsKey(uuid)) {
          this.a(nbttagcompound.getByte("type"), entityhuman.world, uuid,
  ...
  public Packet a(ItemStack itemstack, World world, EntityHuman entityhuman) {
    ...
    return worldmap_worldmaphumantracker == null ? null
        : worldmap_worldmaphumantracker.a(itemstack);
  }
  ...
  public class WorldMapHumanTracker {
    ...
    public Packet a(ItemStack itemstack) {
      // CraftBukkit start
      org.bukkit.craftbukkit.map.RenderData render = WorldMap.this.mapView.render(
          (org.bukkit.craftbukkit.entity.CraftPlayer) this.trackee
      ...
      for (org.bukkit.map.MapCursor cursor : render.cursors) {
        ...
        if (cursor.isVisible()) {
          icons.add(new MapIcon(cursor.getRawType(), cursor.getX(), cursor.getY(),
              cursor.getDirection()));
        }
      }
      ...
      if (this.d) {
        this.d = false;
        return new PacketPlayOutMap(itemstack.getData(), WorldMap.this.scale, icons, render.buffer,
            this.e, this.f, this.g + 1 - this.e, this.h + 1 - this.f);
      } else {
        return this.i++ % 5 == 0 ? new PacketPlayOutMap(itemstack.getData(), WorldMap.this.scale,
            icons, render.buffer, 0, 0, 0, 0) : null;
      }
      // CraftBukkit end
    }
