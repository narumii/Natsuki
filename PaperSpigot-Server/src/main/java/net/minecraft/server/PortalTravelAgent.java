...
public class PortalTravelAgent {
    ...
    for (int l = -2; l <= 2; ++l) {
      for (int i1 = -2; i1 <= 2; ++i1) {
        for (int j1 = -1; j1 < 3; ++j1) {
          ...
          if (this.a.getType(new BlockPosition(k1, l1, i2)).getBlock() != (flag ? Blocks.OBSIDIAN
              : Blocks.AIR)) {
            return null;
          }
        }
  ...
  public boolean b(Entity entity, float f) {
    ...
    Location exit = new Location(this.a.getWorld(), found.getX(), found.getY(), found.getZ(), f,
        entity.pitch);
    ...
    if (entity.motX != velocity.getX() || entity.motY != velocity.getY() || entity.motZ != velocity
        .getZ()) {
  ...
  public BlockPosition findPortal(double x, double y, double z,
    ...
    if (this.c.contains(k)) {
      ...
      for (int l = -searchRadius; l <= searchRadius; ++l) {   // Paper - actually use search radius
        ...
        for (int i1 = -searchRadius; i1 <= searchRadius;
            ++i1) {    // Paper - actually use search radius
          for (BlockPosition blockposition2 = blockposition
              .a(l, this.a.V() - 1 - blockposition.getY(), i1); blockposition2.getY() >= 0;
              blockposition2 = blockposition1) {
            blockposition1 = blockposition2.down();
            if (this.a.getType(blockposition2).getBlock() == Blocks.PORTAL) {
              while (this.a.getType(blockposition1 = blockposition2.down()).getBlock()
                  == Blocks.PORTAL) {
                blockposition2 = blockposition1;
              }
    ...
    if (d0 >= 0.0D) {
      if (flag1) {
        this.c.put(k,
            new PortalTravelAgent.ChunkCoordinatesPortal((BlockPosition) object, this.a.getTime()));
  ...
  public void adjustExit(Entity entity, Location position, Vector velocity) {
    ...
    BlockPosition object = new BlockPosition(position.getBlockX(), position.getBlockY(),
        position.getBlockZ());
    float f = position.getYaw();

    if (this.a.getWorld().getEnvironment() == org.bukkit.World.Environment.THE_END
        || entity.getBukkitEntity().getWorld().getEnvironment()
        == org.bukkit.World.Environment.THE_END || entity.aG() == null) {
      ...
      ShapeDetector.ShapeDetectorCollection shapedetector_shapedetectorcollection = Blocks.PORTAL
          .f(this.a, object);
      boolean flag2 = shapedetector_shapedetectorcollection.b().e().c()
      ...
      f = f - (float) (entity.aH().opposite().b() * 90) + (float) (
    ...
    EntityPortalExitEvent event = new EntityPortalExitEvent(entity.getBukkitEntity(), from,
    ...
    if (event.isCancelled() || to == null || !entity.isAlive()) {
      ...
      velocity.copy(event
          .getAfter()); // event.getAfter() will never be null, as setAfter() will cause an NPE if null is passed in
    }
    // CraftBukkit end
  }
  ...
  public boolean createPortal(double x, double y, double z, int b0) {
              ...
              for (k3 = 0; k3 < 3; ++k3) {
                for (l3 = 0; l3 < 4; ++l3) {
                  for (i4 = -1; i4 < 4; ++i4) {
                    ...
                    if (i4 < 0 && !this.a.getType(blockposition_mutableblockposition).getBlock()
                        .getMaterial().isBuildable() || i4 >= 0 && !this.a
                        .isEmpty(blockposition_mutableblockposition)) {
                      continue label271;
                    }
                  }
                }
      ...
      for (i2 = i - b0; i2 <= i + b0; ++i2) {
        ...
        for (j2 = k - b0; j2 <= k + b0; ++j2) {
          ...
          for (k2 = this.a.V() - 1; k2 >= 0; --k2) {
            if (this.a.isEmpty(blockposition_mutableblockposition.c(i2, k2, j2))) {
              while (k2 > 0 && this.a
                  .isEmpty(blockposition_mutableblockposition.c(i2, k2 - 1, j2))) {
                --k2;
              }
              ...
              for (l2 = l1; l2 < l1 + 2; ++l2) {
                ...
                for (k3 = 0; k3 < 4; ++k3) {
                  for (l3 = -1; l3 < 4; ++l3) {
                    ...
                    if (l3 < 0 && !this.a.getType(blockposition_mutableblockposition).getBlock()
                        .getMaterial().isBuildable() || l3 >= 0 && !this.a
                        .isEmpty(blockposition_mutableblockposition)) {
                      continue label219;
                    }
                  }
    ...
    IBlockData iblockdata = Blocks.PORTAL.getBlockData()
        .set(BlockPortal.AXIS, k5 != 0 ? EnumDirection.EnumAxis.X : EnumDirection.EnumAxis.Z);
