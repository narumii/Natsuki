...
public abstract class EntityHuman extends EntityLiving {
  ...
  protected FoodMetaData foodData = new FoodMetaData(
      this); // CraftBukkit - add "this" to constructor
  protected int bm;
  public float bn;
  public float bo;
  public int bp;
      ...
      for (int j = 0; j < i; ++j) {
        ...
        if (itemstack.usesData()) {
          this.world.addParticle(EnumParticle.ITEM_CRACK, vec3d1.a, vec3d1.b, vec3d1.c, vec3d.a,
              vec3d.b + 0.05D, vec3d.c, Item.getId(itemstack.getItem()), itemstack.getData());
        } else {
          this.world.addParticle(EnumParticle.ITEM_CRACK, vec3d1.a, vec3d1.b, vec3d1.c, vec3d.a,
  ...
  protected void s() {
    if (this.g != null) {
      ...
      PlayerItemConsumeEvent event = new PlayerItemConsumeEvent((Player) this.getBukkitEntity(),
          craftItem);
  ...
  public void m() {
    ...
    float f1 = (float) (org.bukkit.craftbukkit.TrigMath.atan(-this.motY * 0.20000000298023224D)
        * 15.0D); // CraftBukkit
    ...
    if (this.getHealth() > 0.0F && !this.isSpectator()) {
      ...
      if (this.vehicle != null && !this.vehicle.dead) {
        axisalignedbb = this.getBoundingBox().a(this.vehicle.getBoundingBox())
            .grow(1.0D, 0.0D, 1.0D);
      } else {
      ...
      if (this
          .ae()) { // Spigot: Add this.ae() condition (second !this.isDead near bottom of EntityLiving)
  ...
  public void b(Entity entity, int i) {
    ...
    Collection<ScoreboardScore> collection = this.world.getServer().getScoreboardManager()
        .getScoreboardScores(IScoreboardCriteria.f, this.getName(),
            new java.util.ArrayList<ScoreboardScore>());

    if (entity instanceof EntityHuman) {
      this.b(StatisticList.B);
      // CraftBukkit - Get our scores instead
      this.world.getServer().getScoreboardManager()
          .getScoreboardScores(IScoreboardCriteria.e, this.getName(), collection);
    ...
    while (iterator.hasNext()) {
      ScoreboardScore scoreboardscore = (ScoreboardScore) iterator
          .next(); // CraftBukkit - Use our scores instead
  ...
  private Collection e(Entity entity) { // CraftBukkit - TODO: Check me?
    ...
    if (scoreboardteam != null) {
      ...
      if (i >= 0 && i < IScoreboardCriteria.i.length) {
        Iterator iterator = this.getScoreboard().getObjectivesForCriteria(IScoreboardCriteria.i[i])
            .iterator();

        while (iterator.hasNext()) {
          ScoreboardObjective scoreboardobjective = (ScoreboardObjective) iterator.next();
          ScoreboardScore scoreboardscore = this.getScoreboard()
              .getPlayerScoreForObjective(entity.getName(), scoreboardobjective);
  ...
  public EntityItem a(boolean flag) {
    ...
    return this.a(this.inventory.splitStack(this.inventory.itemInHandIndex,
  ...
  public EntityItem a(ItemStack itemstack, boolean flag, boolean flag1) {
    if (itemstack == null) {
      ...
      if (flag) {
        ...
        f = 0.3F;
        entityitem.motX = -MathHelper.sin(this.yaw / 180.0F * 3.1415927F) * MathHelper
            .cos(this.pitch / 180.0F * 3.1415927F) * f;
        entityitem.motZ = MathHelper.cos(this.yaw / 180.0F * 3.1415927F) * MathHelper
            .cos(this.pitch / 180.0F * 3.1415927F) * f;
      ...
      if (event.isCancelled()) {
        ...
        } else if (flag1 && cur.isSimilar(drop.getItemStack())
  ...
  public void b(NBTTagCompound nbttagcompound) {
    ...
    nbttagcompound.setString("SpawnWorld",
        spawnWorld); // CraftBukkit - fixes bed spawns for multiworld worlds
  ...
  public boolean a(EntityHuman entityhuman) {
    ...
    if (entityhuman instanceof EntityPlayer) {
      ...
      team = thatPlayer.getBukkitEntity().getScoreboard()
          .getPlayerTeam(thatPlayer.getBukkitEntity());
      if (team == null || team.allowFriendlyFire()) {
        return true;
      }
    } else {
      // This should never be called, but is implemented anyway
      org.bukkit.OfflinePlayer thisPlayer = entityhuman.world.getServer()
          .getOfflinePlayer(entityhuman.getName());
      team = entityhuman.world.getServer().getScoreboardManager().getMainScoreboard()
  ...
  protected boolean d(DamageSource damagesource, float f) { // void -> boolean
    ...
    if (!this.isInvulnerable(damagesource)) {
      if (!damagesource.ignoresArmor() && this.isBlocking() && f > 0.0F) {
        f = (1.0F + f)
            * this.world.paperSpigotConfig.playerBlockingDamageMultiplier; // PaperSpigot - Configurable damage multiplier for blocking;
      }
      ...
      if (!entity.l(this)) {
        ...
        if (f > 0.0F || f1 > 0.0F) {
          boolean flag = !world.paperSpigotConfig.disablePlayerCrits && this.fallDistance > 0.0F
              && !this.onGround && !this.k_() && !this.V() && !this
              .hasEffect(MobEffectList.BLINDNESS) && this.vehicle == null
              && entity instanceof EntityLiving; // PaperSpigot
          ...
          if (entity instanceof EntityLiving && j > 0 && !entity.isBurning()) {
            ...
            EntityCombustByEntityEvent combustEvent = new EntityCombustByEntityEvent(
                this.getBukkitEntity(), entity.getBukkitEntity(), 1);
          ...
          if (flag2) {
            if (i > 0) {
              entity.g(-MathHelper.sin(this.yaw * 3.1415927F / 180.0F) * (float) i * 0.5F, 0.1D,
            ...
            this.applyExhaustion(
                world.spigotConfig.combatExhaustion); // Spigot - Change to use configurable value
          } else if (flag1) {
            entity.extinguish();
          }
        }
  ...
  public EntityHuman.EnumBedResult a(BlockPosition blockposition) {
    ...
    if (this.getBukkitEntity() instanceof Player) {
      Player player = (Player) this.getBukkitEntity();
      org.bukkit.block.Block bed = this.world.getWorld()
    ...
    if (this.world.isLoaded(blockposition)) {
      ...
      this.setPosition((float) blockposition.getX() + f, (float) blockposition.getY() + 0.6875F,
          (float) blockposition.getZ() + f1);
    } else {
      this.setPosition((float) blockposition.getX() + 0.5F, (float) blockposition.getY() + 0.6875F,
          (float) blockposition.getZ() + 0.5F);
    }
  ...
  public void a(boolean flag, boolean flag1, boolean flag2) {
    ...
    if (this.bx != null && iblockdata.getBlock() == Blocks.BED) {
      this.world
          .setTypeAndData(this.bx, iblockdata.set(BlockBed.OCCUPIED, Boolean.valueOf(false)), 4);
      ...
      this.setPosition((float) blockposition.getX() + 0.5F, (float) blockposition.getY() + 0.1F,
          (float) blockposition.getZ() + 0.5F);
    }
  ...
  private static BlockPosition getBed0(World world, BlockPosition blockposition, boolean flag) {
    ...
    ((ChunkProviderServer) world.chunkProvider)
        .getChunkAt(blockposition.getX() >> 4, blockposition.getZ() >> 4); // CraftBukkit
  ...
  public void bF() {
    ...
    if (this.isSprinting()) {
      this.applyExhaustion(
          world.spigotConfig.sprintExhaustion); // Spigot - Change to use configurable value
    } else {
      this.applyExhaustion(
          world.spigotConfig.walkExhaustion); // Spigot - Change to use configurable value
    }
  ...
  public void checkMovement(double d0, double d1, double d2) {
    if (this.vehicle == null) {
      ...
      if (this.a(Material.WATER)) {
        i = (int) Math.round(Math.sqrt(d0 * d0 + d1 * d1 + d2 * d2) * 100.0F);
        if (i > 0) {
          this.a(StatisticList.p, i);
          this.applyExhaustion(world.paperSpigotConfig.playerSwimmingExhaustion * (float) i
              * 0.01F); // PaperSpigot - Configurable swimming exhaustion
        }
      } else {
        if (this.V()) {
          i = (int) Math.round(Math.sqrt(d0 * d0 + d2 * d2) * 100.0F);
          if (i > 0) {
            this.a(StatisticList.l, i);
            this.applyExhaustion(world.paperSpigotConfig.playerSwimmingExhaustion * (float) i
                * 0.01F); // PaperSpigot - Configurable swimming (diving) exhaustion
          }
        } else if (this.k_()) {
          if (d1 > 0.0D) {
  ...
  private void l(double d0, double d1, double d2) {
    if (this.vehicle != null) {
      int i = (int) Math.round(Math.sqrt(d0 * d0 + d1 * d1 + d2 * d2) * 100.0F);
      ...
      if (i > 0) {
        if (this.vehicle instanceof EntityMinecartAbstract) {
          ...
          } else if (this.e.c(MathHelper.floor(this.locX), (int) Math.floor(this.locY),
              (int) Math.floor(this.locZ)) >= 1000000.0D) {
  ...
  public IChatBaseComponent getScoreboardDisplayName() {
    ...
    chatcomponenttext.getChatModifier().setChatClickable(
        new ChatClickable(ChatClickable.EnumClickAction.SUGGEST_COMMAND,
            "/msg " + this.getName() + " "));
  ...
  public boolean a(ChestLock chestlock) {
    if (chestlock.a()) {
      ...
      return (itemstack != null && itemstack.hasName()) && itemstack.getName()
          .equals(chestlock.b());
    }
  }

  public boolean getSendCommandFeedback() {
    return MinecraftServer.getServer().worldServer[0].getGameRules()
    ...
    if (i >= 0 && i < this.inventory.items.length) {
      ...
      if (j >= 0 && j < this.inventory.armor.length) {
        ...
        if (itemstack != null && itemstack.getItem() != null) {
          ...
          } else if (k != 4 || itemstack.getItem() != Items.SKULL && !(itemstack
