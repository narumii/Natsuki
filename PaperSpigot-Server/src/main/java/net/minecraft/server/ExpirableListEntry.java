...
public abstract class ExpirableListEntry<T> extends JsonListEntry<T> {
  ...
  protected ExpirableListEntry(T t0, JsonObject jsonobject) {
    ...
    try {
      date = jsonobject.has("created") ? ExpirableListEntry.a
          .parse(jsonobject.get("created").getAsString()) : new Date();
    ...
    try {
      date1 = jsonobject.has("expires") ? ExpirableListEntry.a
          .parse(jsonobject.get("expires").getAsString()) : null;
    } catch (ParseException parseexception1) {
      date1 = null;
    }

    this.d = date1;
    this.e = jsonobject.has("reason") ? jsonobject.get("reason").getAsString()
  ...
  protected void a(JsonObject jsonobject) {
    ...
    jsonobject
        .addProperty("expires", this.d == null ? "forever" : ExpirableListEntry.a.format(this.d));
