...
public class UserCache {
  ...
  private static GameProfile a(MinecraftServer minecraftserver, String s) {
    ...
    minecraftserver.getGameProfileRepository()
        .findProfilesByNames(new String[]{s}, Agent.MINECRAFT, profilelookupcallback);
  ...
  private void a(GameProfile gameprofile, Date date) {
    ...
    UserCache.UserCacheEntry usercache_usercacheentry = new UserCache.UserCacheEntry(gameprofile,
        date, null);
  ...
  public GameProfile getProfile(String s) {
    ...
    if (usercache_usercacheentry != null && (new Date()).getTime() >= usercache_usercacheentry.c
        .getTime()) {
  ...
  public void b() {
    ...
    try {
      ...
      while (iterator.hasNext()) {
        UserCache.UserCacheEntry usercache_usercacheentry = (UserCache.UserCacheEntry) iterator
            .next();
      ...
      JsonList.a.warn(
          "Usercache.json is corrupted or has bad formatting. Deleting it to prevent further issues.");
      this.g.delete();
      // Spigot End
  ...
  class BanEntrySerializer implements JsonDeserializer<UserCache.UserCacheEntry>,
      JsonSerializer<UserCache.UserCacheEntry> {
    ...
    public UserCacheEntry deserialize(JsonElement jsonelement, Type type,
        ...
        throws JsonParseException { // CraftBukkit - decompile error
