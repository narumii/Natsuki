...
public class PacketPlayOutMapChunkBulk implements Packet<PacketListenerPlayOut> {
  ...
  public PacketPlayOutMapChunkBulk(List<Chunk> list) {
    ...
    for (int j = 0; j < i; ++j) {
      ...
      PacketPlayOutMapChunk.ChunkMap packetplayoutmapchunk_chunkmap = chunk
          .getChunkMap(true, '\uffff'); // PaperSpigot
  ...
  public void b(PacketDataSerializer packetdataserializer) throws IOException {
    ...
    for (i = 0; i < this.a.length; ++i) {
      world.spigotConfig.antiXrayInstance
          .obfuscate(this.a[i], this.b[i], this.c[i].b, this.c[i].a, world); // Spigot
