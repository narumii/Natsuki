...
public class RecipesBanner {
  ...
  void a(CraftingManager craftingmanager) {
    ...
    for (int j = 0; j < i; ++j) {
      ...
      craftingmanager
          .registerShapedRecipe(new ItemStack(Items.BANNER, 1, enumcolor.getInvColorIndex()), "###",
              "###", " | ", Character.valueOf('#'),
  ...
  static class AddRecipe extends ShapelessRecipes implements
      IRecipe { // CraftBukkit - added extends

    // CraftBukkit start - Delegate to new parent class with bogus info
    private AddRecipe() {
      super(new ItemStack(Items.BANNER, 0, 0),
          java.util.Arrays.asList(new ItemStack(Items.BANNER)));
    }
    // CraftBukkit end
    ...
    public ItemStack craftItem(InventoryCrafting inventorycrafting) {
      ...
      TileEntityBanner.EnumBannerPatternType tileentitybanner_enumbannerpatterntype = this
          .c(inventorycrafting);
    ...
    private TileEntityBanner.EnumBannerPatternType c(InventoryCrafting inventorycrafting) {
      ...
      for (int j = 0; j < i; ++j) {
        ...
        if (tileentitybanner_enumbannerpatterntype.d()) {
          ...
          } else if (inventorycrafting.getSize()
              != tileentitybanner_enumbannerpatterntype.c().length
  ...
  static class DuplicateRecipe extends ShapelessRecipes implements
      IRecipe { // CraftBukkit - added extends

    // CraftBukkit start - Delegate to new parent class with bogus info
    private DuplicateRecipe() {
      super(new ItemStack(Items.BANNER, 0, 0),
          java.util.Arrays.asList(new ItemStack(Items.DYE, 0, 5)));
    }
    // CraftBukkit end
