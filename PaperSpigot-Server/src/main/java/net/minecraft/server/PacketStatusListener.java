...
public class PacketStatusListener implements PacketStatusInListener {
  ...
  private static final IChatBaseComponent a = new ChatComponentText(
  ...
  public void a(PacketStatusInStart packetstatusinstart) {
    ...
    ServerPing.ServerPingPlayerSample playerSample = new ServerPing.ServerPingPlayerSample(
        event.getMaxPlayers(), profiles.size());
    // Spigot Start
    if (!profiles.isEmpty()) {
      java.util.Collections.shuffle(
          profiles); // This sucks, its inefficient but we have no simple way of doing it differently
      profiles = profiles.subList(0, Math.min(profiles.size(),
          org.spigotmc.SpigotConfig.playerSample)); // Cap the sample to n (or less) displayed players, ie: Vanilla behaviour
    }
    // Spigot End
    ...
    ping.setServerInfo(new ServerPing.ServerData(
        minecraftServer.getServerModName() + " " + minecraftServer.getVersion(),
        47)); // TODO: Update when protocol changes
  ...
  public void a(PacketStatusInPing packetstatusinping) {
    if (!this.d) {
      Holder.getBlacklist().add(
          ((java.net.InetSocketAddress) networkManager.getSocketAddress()).getAddress()
              .getHostAddress());
      this.networkManager.close(PacketStatusListener.a);
      return;
    }
