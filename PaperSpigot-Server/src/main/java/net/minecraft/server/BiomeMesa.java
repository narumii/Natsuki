...
public class BiomeMesa extends BiomeBase {
          ...
          if (k1 == -1) {
            ...
            if (l1 >= i1 - 1) {
              if (this.aJ && l1 > 86 + j1 * 2) {
                if (flag) {
                  chunksnapshot.a(l, l1, k, Blocks.DIRT.getBlockData()
                      .set(BlockDirt.VARIANT, BlockDirt.EnumDirtVariant.COARSE_DIRT));
                } else {
                ...
                if (l1 >= 64 && l1 <= 127) {
                  ...
                  iblockdata3 = Blocks.STAINED_HARDENED_CLAY.getBlockData()
                      .set(BlockCloth.COLOR, EnumColor.ORANGE);
                }
            ...
            --k1;
            if (flag1) {
              chunksnapshot.a(l, l1, k, Blocks.STAINED_HARDENED_CLAY.getBlockData()
                  .set(BlockCloth.COLOR, EnumColor.ORANGE));
            } else {
              iblockdata3 = this.a(i, l1, j);
              chunksnapshot.a(l, l1, k, iblockdata3);
  ...
  private void a(long i) {
    ...
    for (j = 0; j < 64; ++j) {
      ...
      if (j < 64) {
        this.aD[j] = Blocks.STAINED_HARDENED_CLAY.getBlockData()
            .set(BlockCloth.COLOR, EnumColor.ORANGE);
      }
    }
    ...
    for (k = 0; k < j; ++k) {
      ...
      for (j1 = 0; i1 + j1 < 64 && j1 < l; ++j1) {
        this.aD[i1 + j1] = Blocks.STAINED_HARDENED_CLAY.getBlockData()
            .set(BlockCloth.COLOR, EnumColor.YELLOW);
      }
    }
    ...
    for (l = 0; l < k; ++l) {
      ...
      for (k1 = 0; j1 + k1 < 64 && k1 < i1; ++k1) {
        this.aD[j1 + k1] = Blocks.STAINED_HARDENED_CLAY.getBlockData()
            .set(BlockCloth.COLOR, EnumColor.BROWN);
      }
    }
    ...
    for (i1 = 0; i1 < l; ++i1) {
      ...
      for (int l1 = 0; k1 + l1 < 64 && l1 < j1; ++l1) {
        this.aD[k1 + l1] = Blocks.STAINED_HARDENED_CLAY.getBlockData()
            .set(BlockCloth.COLOR, EnumColor.RED);
      }
    }
    ...
    for (k1 = 0; k1 < i1; ++k1) {
      ...
      for (int i2 = 0; j1 + i2 < 64 && i2 < b0; ++i2) {
        this.aD[j1 + i2] = Blocks.STAINED_HARDENED_CLAY.getBlockData()
            .set(BlockCloth.COLOR, EnumColor.WHITE);
        if (j1 + i2 > 1 && random.nextBoolean()) {
          this.aD[j1 + i2 - 1] = Blocks.STAINED_HARDENED_CLAY.getBlockData()
              .set(BlockCloth.COLOR, EnumColor.SILVER);
        }
  ...
  private IBlockData a(int i, int j, int k) {
    int l = (int) Math
        .round(this.aH.a((double) i * 1.0D / 512.0D, (double) i * 1.0D / 512.0D) * 2.0D);
