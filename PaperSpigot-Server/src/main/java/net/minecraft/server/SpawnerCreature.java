...
public final class SpawnerCreature {
  ...
  private int getEntityCount(WorldServer server, Class oClass) {
    ...
    while (it.hasNext()) {
      ...
      if (!server.chunkProviderServer.unloadQueue.contains(coord) && server
          .isChunkLoaded(x, z, true)) {
    ...
    if (!flag && !flag1) {
      ...
      while (iterator.hasNext()) {
        ...
        if (!entityhuman.isSpectator() || !entityhuman.affectsSpawning) { // PaperSpigot
          ...
          b0 = (b0 > worldserver.spigotConfig.viewDistance)
              ? (byte) worldserver.spigotConfig.viewDistance : b0;
          b0 = (b0 > 8) ? 8 : b0;
          // Spigot End
                    ...
                    if (i4 < 4) {
                      label108:
                      {
                        ...
                        if (!worldserver.isPlayerNearbyWhoAffectsSpawning(f, k3, f1, 24.0D)
                            && blockposition.c(f, k3, f1)
                            >= 576.0D) { // PaperSpigot - Affects Spawning API
                          ...
                          if (worldserver.a(enumcreaturetype, biomebase_biomemeta, blockposition2)
                              && a(EntityPositionTypes.a(biomebase_biomemeta.b), worldserver,
                              blockposition2)) {
                            ...
                            if (entityinsentient.bR() && entityinsentient.canSpawn()) {
                              groupdataentity = entityinsentient
                                  .prepare(worldserver.E(new BlockPosition(entityinsentient)),
                                      groupdataentity);
                              if (entityinsentient.canSpawn()) {
                                ++l2;
                                worldserver.addEntity(entityinsentient,
                                    SpawnReason.NATURAL); // CraftBukkit - Added a reason for spawning this creature
                              }
  ...
  public static boolean a(
      EntityInsentient.EnumEntityPositionType entityinsentient_enumentitypositiontype, World world,
    ...
    if (!world.getWorldBorder().a(blockposition)) {
      ...
      if (entityinsentient_enumentitypositiontype
          == EntityInsentient.EnumEntityPositionType.IN_WATER) {
        return block.getMaterial().isLiquid() && world.getType(blockposition.down()).getBlock()
            .getMaterial().isLiquid() && !world.getType(blockposition.up()).getBlock()
            .isOccluding();
      } else {
        ...
        if (!World.a(world, blockposition1)) {
          ...
          return flag && !block.isOccluding() && !block.getMaterial().isLiquid() && !world
  ...
  public static void a(World world, BiomeBase biomebase, int i, int j, int k, int l,
      Random random) {
    List list = biomebase.getMobs(EnumCreatureType.CREATURE);

    if (!list.isEmpty()) {
      while (random.nextFloat() < biomebase.g()) {
        BiomeBase.BiomeMeta biomebase_biomemeta = (BiomeBase.BiomeMeta) WeightedRandom
            .a(world.random, list);
        int i1 = biomebase_biomemeta.c + random
            .nextInt(1 + biomebase_biomemeta.d - biomebase_biomemeta.c);
        GroupDataEntity groupdataentity = null;
        ...
        for (int j2 = 0; j2 < i1; ++j2) {
          ...
          for (int k2 = 0; !flag && k2 < 4; ++k2) {
            ...
            if (a(EntityInsentient.EnumEntityPositionType.ON_GROUND, world, blockposition)) {
              ...
              entityinsentient
                  .setPositionRotation((float) j1 + 0.5F, blockposition.getY(), (float) k1 + 0.5F,
                      random.nextFloat() * 360.0F, 0.0F);
              // CraftBukkit start - Added a reason for spawning this creature, moved entityinsentient.prepare(groupdataentity) up
              groupdataentity = entityinsentient
                  .prepare(world.E(new BlockPosition(entityinsentient)), groupdataentity);
            ...
            for (k1 += random.nextInt(5) - random.nextInt(5);
                j1 < i || j1 >= i + k || k1 < j || k1 >= j + k;
                k1 = i2 + random.nextInt(5) - random.nextInt(5)) {
