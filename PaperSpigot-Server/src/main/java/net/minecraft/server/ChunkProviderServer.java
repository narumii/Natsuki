...
public class ChunkProviderServer implements IChunkProvider {
  ...
  public ChunkProviderServer(WorldServer worldserver, IChunkLoader ichunkloader,
  ...
  public void queueUnload(int i, int j) {
    ...
    if (chunk != null && chunk.world.paperSpigotConfig.useAsyncLighting && (
        chunk.pendingLightUpdates.get() > 0
            || chunk.world.getTime() - chunk.lightUpdateTime < 20)) {
      return;
    }
    // PaperSpigot end
      ...
      if (chunk == null) {
        if (this.chunkProvider == null) {
          ...
          try {
            ...
            crashreportsystemdetails.a("Position hash",
                Long.valueOf(LongHash.toLong(i, j))); // CraftBukkit - Use LongHash
  ...
  public Chunk getOrCreateChunk(int i, int j) {
    ...
    chunk = chunk == null ? (!this.world.ad() && !this.forceChunkLoad ? this.emptyChunk
        : this.getChunkAt(i, j)) : chunk;
    ...
    if (i != chunk.locX || j != chunk.locZ) {
      // Paper start
      String msg = "Chunk (" + chunk.locX + ", " + chunk.locZ + ") stored at  (" + i + ", " + j
          + ") in world '" + world.getWorld().getName() + "'";
      b.error(msg);
  ...
  public void saveChunk(Chunk chunk) {
    if (this.chunkLoader != null) {
      try {
        ...
        ChunkProviderServer.b
            .error("Couldn\'t save chunk; already in use by another instance of Minecraft?",
  ...
  public List<BiomeBase.BiomeMeta> getMobsFor(EnumCreatureType enumcreaturetype,
