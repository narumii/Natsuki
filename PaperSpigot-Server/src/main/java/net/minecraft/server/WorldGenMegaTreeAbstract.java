...
public abstract class WorldGenMegaTreeAbstract extends WorldGenTreeAbstract {
  ...
  public WorldGenMegaTreeAbstract(boolean flag, int i, int j, IBlockData iblockdata,
          ...
          for (int l = -b0; l <= b0 && flag; ++l) {
            if (blockposition.getY() + j < 0 || blockposition.getY() + j >= 256 || (
                !this.a(world.getType(blockposition.a(k, j, l)).getBlock())
                    && world.getType(blockposition.a(k, j, l)).getBlock()
                    != Blocks.SAPLING)) { // CraftBukkit - ignore our own saplings
              flag = false;
            }
          }
        }
  ...
  protected void a(World world, BlockPosition blockposition, int i) {
    ...
    for (int k = -i; k <= i + 1; ++k) {
      for (int l = -i; l <= i + 1; ++l) {
        ...
        if (k * k + l * l <= j || i1 * i1 + j1 * j1 <= j || k * k + j1 * j1 <= j
            || i1 * i1 + l * l <= j) {
