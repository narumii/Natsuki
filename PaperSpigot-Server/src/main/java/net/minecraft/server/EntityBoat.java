...
public class EntityBoat extends Entity {
  ...
  public void collide(Entity entity) {
    ...
    VehicleEntityCollisionEvent event = new VehicleEntityCollisionEvent(
        (Vehicle) this.getBukkitEntity(), hitEntity);
  ...
  public void t_() {
    ...
    for (int i = 0; i < b0; ++i) {
      double d1 = this.getBoundingBox().b
          + (this.getBoundingBox().e - this.getBoundingBox().b) * (double) (i + 0) / (double) b0
          - 0.125D;
      double d2 = this.getBoundingBox().b
          + (this.getBoundingBox().e - this.getBoundingBox().b) * (double) (i + 1) / (double) b0
          - 0.125D;
      AxisAlignedBB axisalignedbb = new AxisAlignedBB(this.getBoundingBox().a, d1,
          this.getBoundingBox().c, this.getBoundingBox().d, d2, this.getBoundingBox().f);
    ...
    if (this.world.isClientSide && this.a) {
      if (this.c > 0) {
        ...
        this.pitch = (float) ((double) this.pitch
            + (this.h - (double) this.pitch) / (double) this.c);
        --this.c;
      ...
      if (this.passenger instanceof EntityLiving) {
        ...
        this.motX += -Math.sin(f * 3.1415927F / 180.0F) * this.b * (double) entityliving.ba
            * 0.05000000074505806D;
        this.motZ += Math.cos(f * 3.1415927F / 180.0F) * this.b * (double) entityliving.ba
            * 0.05000000074505806D;
      }
  ...
  public void al() {
    if (this.passenger != null) {
      ...
      this.passenger
          .setPosition(this.locX + d0, this.locY + this.an() + this.passenger.am(), this.locZ + d1);
    }
  }
  ...
  protected void a(double d0, boolean flag, Block block, BlockPosition blockposition) {
    ...
    } else if (this.world.getType((new BlockPosition(this)).down()).getBlock().getMaterial()
        != Material.WATER && d0 < 0.0D) {
