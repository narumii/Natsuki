...
public class CommandGamerule extends CommandAbstract {
  ...
  public void execute(ICommandListener icommandlistener, String[] astring) throws CommandException {
    GameRules gamerules = icommandlistener.getWorld()
        .getGameRules(); // CraftBukkit - Use current world
    ...
    switch (astring.length) {
      ...
      default:
        if (gamerules.a(s, GameRules.EnumGameRuleType.BOOLEAN_VALUE) && !"true".equals(s1)
            && !"false".equals(s1)) {
    ...
    if ("reducedDebugInfo".equals(s)) {
      ...
      while (iterator.hasNext()) {
        ...
        entityplayer.playerConnection
            .sendPacket(new PacketPlayOutEntityStatus(entityplayer, (byte) i));
      }
    }
