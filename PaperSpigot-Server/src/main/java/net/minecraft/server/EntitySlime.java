...
public class EntitySlime extends EntityInsentient implements IMonster {
  ...
  public EntitySlime(World world) {
    ...
    this.targetSelector
        .a(3, new PathfinderGoalNearestAttackableTargetInsentient(this, EntityIronGolem.class));
  }
  ...
  public void die() {
    ...
    if (!this.world.isClientSide && i > 1 && this.getHealth() <= 0.0F) {
      ...
      // CraftBukkit start
      SlimeSplitEvent event = new SlimeSplitEvent((org.bukkit.entity.Slime) this.getBukkitEntity(),
          j);
      ...
      for (int k = 0; k < j; ++k) {
        ...
        entityslime
            .setPositionRotation(this.locX + (double) f, this.locY + 0.5D, this.locZ + (double) f1,
                this.random.nextFloat() * 360.0F, 0.0F);
        this.world.addEntity(entityslime,
            org.bukkit.event.entity.CreatureSpawnEvent.SpawnReason.SLIME_SPLIT); // CraftBukkit - SpawnReason
      }
    }
  ...
  protected void e(EntityLiving entityliving) {
    ...
    if (this.hasLineOfSight(entityliving)
        && this.h(entityliving) < 0.6D * (double) i * 0.6D * (double) i && entityliving
        .damageEntity(DamageSource.mobAttack(this), (float) this.cj())) {
  ...
  public boolean bR() {
    BlockPosition blockposition = new BlockPosition(MathHelper.floor(this.locX), 0,
    ...
    if (this.world.getWorldData().getType() == WorldType.FLAT && this.random.nextInt(4) != 1) {
      ...
      if (this.world.getDifficulty() != EnumDifficulty.PEACEFUL) {
        ...
        boolean isSlimeChunk =
            world.paperSpigotConfig.allChunksAreSlimeChunks || chunk.a(987234911L).nextInt(10) == 0;
  ...
  static class ControllerMoveSlime extends ControllerMove {
    ...
    public void c() {
      ...
      if (!this.f) {
        ...
        this.f = false;
        if (this.a.onGround) {
          this.a.k((float) (this.e * this.a.getAttributeInstance(GenericAttributes.MOVEMENT_SPEED)
              .getValue()));
          if (this.h-- <= 0) {
            this.h = this.i.cg();
            if (this.j) {
