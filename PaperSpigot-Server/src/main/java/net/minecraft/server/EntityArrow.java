...
public class EntityArrow extends Entity implements IProjectile {
  ...
  public EntityArrow(World world, EntityLiving entityliving, EntityLiving entityliving1, float f,
      float f1) {
    super(world);
    this.j = 10.0D;
    ...
    double d1 =
        entityliving1.getBoundingBox().b + (double) (entityliving1.length / 3.0F) - this.locY;
  ...
  public EntityArrow(World world, EntityLiving entityliving, float f) {
    ...
    this.setSize(0.5F, 0.5F);
    this.setPositionRotation(entityliving.locX,
        entityliving.locY + (double) entityliving.getHeadHeight(), entityliving.locZ,
    ...
    this.motX = -MathHelper.sin(this.yaw / 180.0F * 3.1415927F) * MathHelper
        .cos(this.pitch / 180.0F * 3.1415927F);
  ...
  public void shoot(double d0, double d1, double d2, float f, float f1) {
    ...
    d1 /= f2;
    d2 /= f2;
    d0 += this.random.nextGaussian() * (double) (this.random.nextBoolean() ? -1 : 1)
        * 0.007499999832361937D * (double) f1;
    d1 += this.random.nextGaussian() * (double) (this.random.nextBoolean() ? -1 : 1)
        * 0.007499999832361937D * (double) f1;
    d2 += this.random.nextGaussian() * (double) (this.random.nextBoolean() ? -1 : 1)
        * 0.007499999832361937D * (double) f1;
    d0 *= f;
    d1 *= f;
    d2 *= f;
  ...
  public void t_() {
    ...
    if (this.lastPitch == 0.0F && this.lastYaw == 0.0F) {
      ...
      this.lastYaw = this.yaw = (float) (MathHelper.b(this.motX, this.motZ) * 180.0D
          / 3.1415927410125732D);
      this.lastPitch = this.pitch = (float) (MathHelper.b(this.motY, f) * 180.0D
          / 3.1415927410125732D);
    }
    ...
    if (this.inGround) {
      ...
      MovingObjectPosition movingobjectposition = this.world
          .rayTrace(vec3d, vec3d1, false, true, false);
      ...
      if (movingobjectposition != null && movingobjectposition.entity != null
        ...
        if (entityhuman.abilities.isInvulnerable
            || this.shooter instanceof EntityHuman && !((EntityHuman) this.shooter)
            .a(entityhuman)) {
      ...
      if (movingobjectposition != null) {
        ...
        if (movingobjectposition.entity != null) {
          f2 = (float) Math
              .sqrt(this.motX * this.motX + this.motY * this.motY + this.motZ * this.motZ);
          ...
          if (movingobjectposition.entity.damageEntity(damagesource, (float) k)) {
            if (this.isBurning() && !(movingobjectposition.entity instanceof EntityEnderman) && (
                !(movingobjectposition.entity instanceof EntityPlayer)
                    ...
                    || this.world.pvpMode)) { // CraftBukkit - abide by pvp setting if destination is a player
              ...
              if (this.knockbackStrength > 0) {
                ...
                if (f3 > 0.0F) {
                  movingobjectposition.entity
                      .g(this.motX * (double) this.knockbackStrength * 0.6000000238418579D
                              / (double) f3, 0.1D,
                          this.motZ * (double) this.knockbackStrength * 0.6000000238418579D
                              / (double) f3);
                }
              }
      ...
      if (this.isCritical()) {
        for (j = 0; j < 4; ++j) {
          this.world.addParticle(EnumParticle.CRIT, this.locX + this.motX * (double) j / 4.0D,
              ...
              -this.motX, -this.motY + 0.2D, -this.motZ);
        }
      }
      ...
      for (this.pitch = (float) (MathHelper.b(this.motY, f2) * 180.0D / 3.1415927410125732D);
