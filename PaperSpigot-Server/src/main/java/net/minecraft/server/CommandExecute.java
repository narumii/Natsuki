    ...
    if (astring.length < 5) {
      ...
      ICommandListener icommandlistener1 = new ICommandListener() {
        ...
        public boolean getSendCommandFeedback() {
          ...
          return minecraftserver == null || minecraftserver.worldServer[0].getGameRules()
      ...
      try {
        ...
        if (icommandlistener instanceof DedicatedServer) {
          ...
          throw new CommandException(
              "Unhandled executor " + icommandlistener.getClass().getSimpleName());
        }
  ...
  public List<String> tabComplete(ICommandListener icommandlistener, String[] astring,
    ...
    return astring.length == 1 ? a(astring, MinecraftServer.getServer().getPlayers())
        : (astring.length > 1 && astring.length <= 4 ? a(astring, 1, blockposition)
            : (astring.length > 5 && astring.length <= 8 && "detect".equals(astring[4]) ? a(astring,
