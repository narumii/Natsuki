...
public class EntityWither extends EntityMonster implements IRangedEntity {
  ...
  public EntityWither(World world) {
    ...
    this.targetSelector.a(2,
        new PathfinderGoalNearestAttackableTarget(this, EntityInsentient.class, 0, false, false,
            EntityWither.bq));
    this.b_ = 50;
  }
  ...
  public void m() {
    ...
    for (j = 0; j < 3; ++j) {
      ...
      this.world.addParticle(EnumParticle.SMOKE_NORMAL,
      ...
      if (flag && this.world.random.nextInt(4) == 0) {
        this.world.addParticle(EnumParticle.SPELL_MOB,
            d8 + this.random.nextGaussian() * 0.30000001192092896D,
            d9 + this.random.nextGaussian() * 0.30000001192092896D,
            d10 + this.random.nextGaussian() * 0.30000001192092896D, 0.699999988079071D,
    ...
    if (this.cl() > 0) {
      for (j = 0; j < 3; ++j) {
        this.world
            .addParticle(EnumParticle.SPELL_MOB, this.locX + this.random.nextGaussian() * 1.0D,
                this.locY + (double) (this.random.nextFloat() * 3.3F),
                this.locZ + this.random.nextGaussian() * 1.0D, 0.699999988079071D,
      ...
      if (i <= 0) {
        ...
        if (!event.isCancelled()) {
          this.world.createExplosion(this, this.locX, this.locY + (double) this.getHeadHeight(),
              this.locZ, event.getRadius(), event.getFire(),
        ...
        for (EntityPlayer player : MinecraftServer.getServer().getPlayerList().players) {
          ...
          if (world.spigotConfig.witherSpawnSoundRadius > 0 && distanceSquared
      ...
      for (i = 1; i < 3; ++i) {
        if (this.ticksLived >= this.bn[i - 1]) {
          ...
          if (this.world.getDifficulty() == EnumDifficulty.NORMAL
            ...
            if (l > 15) {
              ...
              double d1 = MathHelper
                  .a(this.random, this.locY - (double) f1, this.locY + (double) f1);
          ...
          if (j > 0) {
            ...
            if (entity != null && entity.isAlive() && this.h(entity) <= 900.0D && this
            ...
            List list = this.world
                .a(EntityLiving.class, this.getBoundingBox().grow(20.0D, 8.0D, 20.0D),
                    Predicates.and(EntityWither.bq, IEntitySelector.d));

            for (int i1 = 0; i1 < 10 && !list.isEmpty(); ++i1) {
              EntityLiving entityliving = (EntityLiving) list.get(this.random.nextInt(list.size()));

              if (entityliving != this && entityliving.isAlive() && this
                  .hasLineOfSight(entityliving)) {
  ...
  public boolean damageEntity(DamageSource damagesource, float f) {
    if (this.isInvulnerable(damagesource)) {
      return false;
    } else if (damagesource != DamageSource.DROWN && !(damagesource
        .getEntity() instanceof EntityWither)) {
  ...
  protected void dropDeathLoot(boolean flag, int i) {
    ...
    if (!this.world.isClientSide) {
      Iterator iterator = this.world
          .a(EntityHuman.class, this.getBoundingBox().grow(50.0D, 100.0D, 50.0D)).iterator();
