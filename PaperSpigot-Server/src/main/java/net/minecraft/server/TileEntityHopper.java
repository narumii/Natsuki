...
public class TileEntityHopper extends TileEntityContainer implements IHopper, IUpdatePlayerListBox {
  ...
  public static boolean a(IHopper ihopper) {
    ...
    if (iinventory != null) {
      ...
      Iterator iterator = a(ihopper.getWorld(), ihopper.A(), ihopper.B() + 1.0D, ihopper.C())
          .iterator();
  ...
  private static boolean a(IHopper ihopper, IInventory iinventory, int i,
    ...
    if (itemstack != null && b(iinventory, itemstack, i, enumdirection)) {
      ...
      CraftItemStack oitemstack = CraftItemStack.asCraftMirror(
          iinventory.splitStack(i, ihopper.getWorld().spigotConfig.hopperAmount)); // Spigot
      ...
      if (event.isCancelled()) {
        ...
        if (ihopper instanceof TileEntityHopper) {
          ...
          ((EntityMinecartHopper) ihopper)
              .m(ihopper.getWorld().spigotConfig.hopperTransfer / 2); // Spigot
        }
        return false;
      }
      int origCount = event.getItem().getAmount(); // Spigot
  ...
  private static boolean a(IInventory iinventory, ItemStack itemstack, int i,
      EnumDirection enumdirection) {
  ...
  private static boolean b(IInventory iinventory, ItemStack itemstack, int i,
      EnumDirection enumdirection) {
    return !(iinventory instanceof IWorldInventory) || ((IWorldInventory) iinventory)
        .canTakeItemThroughFace(i, itemstack, enumdirection);
  }

  private static ItemStack c(IInventory iinventory, ItemStack itemstack, int i,
      EnumDirection enumdirection) {
  ...
  private static boolean a(ItemStack itemstack, ItemStack itemstack1) {
    return itemstack.getItem() == itemstack1.getItem() && (
        itemstack.getData() == itemstack1.getData() && (
            itemstack.count <= itemstack.getMaxStackSize() && ItemStack
