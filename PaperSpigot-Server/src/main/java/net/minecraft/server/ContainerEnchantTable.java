...
public class ContainerEnchantTable extends Container {
  ...
  public ContainerEnchantTable(PlayerInventory playerinventory, World world,
    ...
    if (iinventory == this.enchantSlots) {
      ...
      if (itemstack != null) { // CraftBukkit - relax condition
        if (!this.world.isClientSide) {
          ...
          for (j = -1; j <= 1; ++j) {
            for (int k = -1; k <= 1; ++k) {
              if ((j != 0 || k != 0) && this.world.isEmpty(this.position.a(k, 0, j)) && this.world
                  .isEmpty(this.position.a(k, 1, j))) {
      ...
      if (!this.world.isClientSide) {
        ...
        if (list != null) {
          ...
          EnchantItemEvent event = new EnchantItemEvent((Player) entityhuman.getBukkitEntity(),
              this.getBukkitView(),
              this.world.getWorld().getBlockAt(position.getX(), position.getY(), position.getZ()),
              item, this.costs[i], enchants, i);
          this.world.getServer().getPluginManager().callEvent(event);

          int level = event.getExpLevelCost();
          if (event.isCancelled() || (level > entityhuman.expLevel
              && !entityhuman.abilities.canInstantlyBuild) || event.getEnchantsToAdd().isEmpty()) {
            return false;
          }
          if (flag) {
          ...
          for (Map.Entry<org.bukkit.enchantments.Enchantment, Integer> entry : event
  ...
  public boolean a(EntityHuman entityhuman) {
    ...
    return this.world.getType(this.position).getBlock() == Blocks.ENCHANTING_TABLE && entityhuman
