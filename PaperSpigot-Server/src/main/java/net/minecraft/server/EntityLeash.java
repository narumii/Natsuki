...
public class EntityLeash extends EntityHanging {
  ...
  public boolean e(EntityHuman entityhuman) {
        ...
        if (entityinsentient.cc() && entityinsentient.getLeashHolder() == entityhuman) {
          ...
          if (CraftEventFactory.callPlayerLeashEntityEvent(entityinsentient, this, entityhuman)
            ...
            ((EntityPlayer) entityhuman).playerConnection.sendPacket(
                new PacketPlayOutAttachEntity(1, entityinsentient,
    ...
    if (!this.world.isClientSide && !flag) {
      ...
      // CraftBukkit end
      if (true
          || entityhuman.abilities.canInstantlyBuild) { // CraftBukkit - Process for non-creative as well
        d0 = 7.0D;
        list = this.world.a(EntityInsentient.class,
            new AxisAlignedBB(this.locX - d0, this.locY - d0, this.locZ - d0, this.locX + d0,
                this.locY + d0, this.locZ + d0));
        ...
        while (iterator.hasNext()) {
          ...
          if (entityinsentient.cc() && entityinsentient.getLeashHolder() == this) {
            ...
            entityinsentient.unleash(true,
                !entityhuman.abilities.canInstantlyBuild); // false -> survival mode boolean
            // CraftBukkit end
          }
        }
      }
  ...
  public static EntityLeash b(World world, BlockPosition blockposition) {
    ...
    List list = world.a(EntityLeash.class,
        new AxisAlignedBB((double) i - 1.0D, (double) j - 1.0D, (double) k - 1.0D,
