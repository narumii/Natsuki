...
public class BlockFire extends Block {
  ...
  public IBlockData updateState(IBlockData iblockdata, IBlockAccess iblockaccess,
    ...
    if (!World.a(iblockaccess, blockposition.down()) && !Blocks.FIRE
        .e(iblockaccess, blockposition.down())) {
      ...
      return iblockdata
          .set(BlockFire.NORTH, Boolean.valueOf(this.e(iblockaccess, blockposition.north())))
    ...
    if (world.getGameRules().getBoolean("doFireTick")) {
      ...
      if (!flag && world.S() && this.e(world, blockposition)) {
        ...
        if (!flag) {
          ...
          if (!this.e((IBlockAccess) world, blockposition.down()) && i == 15
              && random.nextInt(4) == 0) {
            fireExtinguished(world, blockposition); // CraftBukkit
            return;
          }
            ...
            for (int l = -1; l <= 4; ++l) {
              if (j != 0 || l != 0 || k != 0) {
                ...
                if (j1 > 0) {
                  ...
                  if (k1 > 0 && random.nextInt(i1) <= k1 && (!world.S() || !this
                      .e(world, blockposition1))) {
                    ...
                    if (world.getType(blockposition1) != Blocks.FIRE) {
                      if (CraftEventFactory
                          .callBlockIgniteEvent(world, blockposition1.getX(), blockposition1.getY(),
                              blockposition1.getZ(), blockposition.getX(), blockposition.getY(),
                              blockposition.getZ()).isCancelled()) {
                        continue;
                      }
  ...
  protected boolean e(World world, BlockPosition blockposition) {
    return world.isRainingAt(blockposition) || world.isRainingAt(blockposition.west()) || world
        .isRainingAt(blockposition.east()) || world.isRainingAt(blockposition.north()) || world
        .isRainingAt(blockposition.south());
  }
  ...
  private void a(World world, BlockPosition blockposition, int i, Random random, int j) {
    ...
    if (random.nextInt(i) < k) {
      ...
      // CraftBukkit start
      org.bukkit.block.Block theBlock = world.getWorld()
      ...
      if (random.nextInt(j + 10) < 5 && !world.isRainingAt(blockposition)) {
        ...
        world.setTypeAndData(blockposition,
            this.getBlockData().set(BlockFire.AGE, Integer.valueOf(l)), 3);
      } else {
        fireExtinguished(world, blockposition); // CraftBukkit
      }
  ...
  protected BlockStateList getStateList() {
    return new BlockStateList(this, BlockFire.AGE, BlockFire.NORTH, BlockFire.EAST, BlockFire.SOUTH,
        BlockFire.WEST, BlockFire.UPPER, BlockFire.FLIP, BlockFire.ALT);
  }

  // CraftBukkit start
  private void fireExtinguished(World world, BlockPosition position) {
    if (!CraftEventFactory.callBlockFadeEvent(
        world.getWorld().getBlockAt(position.getX(), position.getY(), position.getZ()), Blocks.AIR)
        .isCancelled()) {
