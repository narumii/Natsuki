...
public class TileEntityFurnace extends TileEntityContainer implements IUpdatePlayerListBox,
    IWorldInventory {
  ...
  public void setItem(int i, ItemStack itemstack) {
    boolean flag = itemstack != null && itemstack.doMaterialsMatch(this.items[i]) && ItemStack
        .equals(itemstack, this.items[i]);
  ...
  public void c() {
    boolean flag = (this.w()
        == Blocks.LIT_FURNACE); // CraftBukkit - SPIGOT-844 - Check if furnace block is lit using the block instead of burn time // PAIL: Rename
    boolean flag1 = false;
      ...
      if (!this.isBurning() && (this.items[1] == null || this.items[0] == null)) {
        ...
        if (this.burnTime <= 0 && this.canBurn()) { // CraftBukkit - == to <=
          ...
          FurnaceBurnEvent furnaceBurnEvent = new FurnaceBurnEvent(
              this.world.getWorld().getBlockAt(position.getX(), position.getY(), position.getZ()),
              fuel, fuelTime(this.items[1]));
  ...
  private boolean canBurn() {
    if (this.items[0] == null) {
      ...
      return itemstack != null && (this.items[2] == null || (
          this.items[2].doMaterialsMatch(itemstack) && (
              this.items[2].count + itemstack.count <= this.getMaxStackSize()
                  ...
                  || this.items[2].count + itemstack.count <= itemstack.getMaxStackSize())));
    }
  }
