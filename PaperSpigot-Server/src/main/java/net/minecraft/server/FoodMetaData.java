...
public class FoodMetaData {
  ...
  public void eat(int i, float f) {
    ...
    this.saturationLevel = Math
        .min(this.saturationLevel + (float) i * f * 2.0F, (float) this.foodLevel);
  }
  ...
  public void a(EntityHuman entityhuman) {
    ...
    if (this.exhaustionLevel > 4.0F) {
      ...
      if (this.saturationLevel > 0.0F) {
        ...
        ((EntityPlayer) entityhuman).playerConnection.sendPacket(new PacketPlayOutUpdateHealth(
            ((EntityPlayer) entityhuman).getBukkitEntity().getScaledHealth(), this.foodLevel,
    ...
    if (entityhuman.world.getGameRules().getBoolean("naturalRegeneration") && this.foodLevel >= 18
      ...
      if (this.foodTickTimer >= 80) {
        ...
        this.a(
            entityhuman.world.spigotConfig.regenExhaustion); // Spigot - Change to use configurable value
