...
public class ItemBoat extends Item {
  ...
  public ItemStack a(ItemStack itemstack, World world, EntityHuman entityhuman) {
    ...
    double d1 = entityhuman.lastY + (entityhuman.locY - entityhuman.lastY) * (double) f
    ...
    if (movingobjectposition == null) {
      ...
      List list = world.getEntities(entityhuman,
          entityhuman.getBoundingBox().a(vec3d2.a * d3, vec3d2.b * d3, vec3d2.c * d3)
      ...
      for (int i = 0; i < list.size(); ++i) {
        ...
        if (entity.ad()) {
          ...
          AxisAlignedBB axisalignedbb = entity.getBoundingBox()
              .grow((double) f10, (double) f10, (double) f10);
      ...
      if (flag) {
        ...
        if (movingobjectposition.type == MovingObjectPosition.EnumMovingObjectType.BLOCK) {
          ...
          EntityBoat entityboat = new EntityBoat(world,
              (double) ((float) blockposition.getX() + 0.5F),
              (double) ((float) blockposition.getY() + 1.0F),
              (double) ((float) blockposition.getZ() + 0.5F));

          entityboat.yaw = (float) (
              ((MathHelper.floor((double) (entityhuman.yaw * 4.0F / 360.0F) + 0.5D) & 3) - 1) * 90);
          if (!world.getCubes(entityboat, entityboat.getBoundingBox().grow(-0.1D, -0.1D, -0.1D))
              .isEmpty()) {
            return itemstack;
          }
