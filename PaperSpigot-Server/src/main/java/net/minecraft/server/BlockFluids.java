...
public abstract class BlockFluids extends Block {
  ...
  protected int e(IBlockAccess iblockaccess, BlockPosition blockposition) {
    return iblockaccess.getType(blockposition).getBlock().getMaterial() == this.material
        ? iblockaccess.getType(blockposition).get(BlockFluids.LEVEL).intValue() : -1;
  }
  ...
  public boolean b(IBlockAccess iblockaccess, BlockPosition blockposition,
    ...
    return material != this.material && (enumdirection == EnumDirection.UP || (
        material != Material.ICE && super.b(iblockaccess, blockposition, enumdirection)));
  }
  ...
  protected Vec3D h(IBlockAccess iblockaccess, BlockPosition blockposition) {
    ...
    while (iterator.hasNext()) {
      ...
      if (j < 0) {
        if (!iblockaccess.getType(blockposition1).getBlock().getMaterial().isSolid()) {
          ...
          if (j >= 0) {
            k = j - (i - 8);
            vec3d = vec3d.add((double) ((blockposition1.getX() - blockposition.getX()) * k),
                (double) ((blockposition1.getY() - blockposition.getY()) * k),
                (double) ((blockposition1.getZ() - blockposition.getZ()) * k));
          }
        }
      } else if (j >= 0) {
        k = j - i;
    ...
    if (iblockaccess.getType(blockposition).get(BlockFluids.LEVEL).intValue() >= 8) {
      ...
      while (iterator.hasNext()) {
        enumdirection = (EnumDirection) iterator.next();
        blockposition1 = blockposition.shift(enumdirection);
        if (this.b(iblockaccess, blockposition1, enumdirection) || this
            .b(iblockaccess, blockposition1.up(), enumdirection)) {
    ...
    if (this.material == Material.LAVA) {
      ...
      for (int j = 0; j < i; ++j) {
        ...
        if (enumdirection != EnumDirection.DOWN
            && world.getType(blockposition.shift(enumdirection)).getBlock().getMaterial()
            == Material.WATER) {
          flag = true;
          break;
        }
  ...
  protected void fizz(World world, BlockPosition blockposition) {
    ...
    for (int i = 0; i < 8; ++i) {
      world.addParticle(EnumParticle.SMOKE_LARGE, d0 + Math.random(), d1 + 1.2D, d2 + Math.random(),
          0.0D, 0.0D, 0.0D);
    }
