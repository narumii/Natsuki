...
public class HandshakeListener implements PacketHandshakingInListener {
  ...
  private boolean isProxy(final InetAddress address) {
    try {
      ...
      if (Natsuki.getInstance().getConfig().API.proxyCheckKey.equalsIgnoreCase("Your Key")
          || Natsuki.getInstance().getConfig().API.proxyCheckKey.equalsIgnoreCase("none")) {
        url = "http://api.stopforumspam.org/api?ip=%address%&json"
            .replace("%address%", address.getHostAddress());
      } else {
        url = "http://proxycheck.io/v2/%address%?&vpn=1&asn=1&risk=1&key=%key%"
            .replace("%address%", address.getHostAddress())
  ...
  public void a(PacketHandshakingInSetProtocol packethandshakinginsetprotocol) {
    switch (HandshakeListener.SyntheticClass_1.a[packethandshakinginsetprotocol.a().ordinal()]) {
      case 1:
        ...
        try {
          ...
          synchronized (throttleTracker) {
            if (throttleTracker.containsKey(address) && !"127.0.0.1"
          ...
          org.apache.logging.log4j.LogManager.getLogger()
              .debug("Failed to check connection throttle", t);
        }
        // CraftBukkit end

        if (packethandshakinginsetprotocol.b() > 47) {
          chatcomponenttext = new ChatComponentText(java.text.MessageFormat
              .format(org.spigotmc.SpigotConfig.outdatedServerMessage.replaceAll("'", "''"),
                  "1.8.8")); // Spigot
          this.b.handle(new PacketLoginOutDisconnect(chatcomponenttext));
          this.b.close(chatcomponenttext);
        } else if (packethandshakinginsetprotocol.b() < 47) {
          chatcomponenttext = new ChatComponentText(java.text.MessageFormat
              .format(org.spigotmc.SpigotConfig.outdatedClientMessage.replaceAll("'", "''"),
                  "1.8.8")); // Spigot
          this.b.handle(new PacketLoginOutDisconnect(chatcomponenttext));
          this.b.close(chatcomponenttext);
        } else {
          ...
          if (org.spigotmc.SpigotConfig.bungee) {
            ...
            if (split.length == 4) {
              b.spoofedProfile = gson
                  .fromJson(split[3], com.mojang.authlib.properties.Property[].class);
            }
          }
          // Spigot End
          ((LoginListener) this.b.getPacketListener()).hostname =
              packethandshakinginsetprotocol.hostname + ":"
      ...
      default:
        Holder.getBlacklist()
            .add(((java.net.InetSocketAddress) b.getSocketAddress()).getAddress().getHostAddress());
        this.b.channel.close();
        throw new UnsupportedOperationException(
            "Invalid intention " + packethandshakinginsetprotocol.a());
    }
