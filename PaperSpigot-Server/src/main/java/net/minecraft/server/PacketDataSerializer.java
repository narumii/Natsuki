...
public class PacketDataSerializer extends ByteBuf {
  ...
  public byte[] readByteArray(int limit) {
    ...
    if (len > 500) {
      throw new DecoderException(
          "The received a byte array longer than allowed " + len + " > " + 500);
    }
    ...
    if (b0 == 0) {
      ...
      try {
        return NBTCompressedStreamTools.a(new ByteBufInputStream(this),
            new NBTReadLimiter(
                (Natsuki.getInstance().getConfig().PACKET.NBT.maxNbtSize == -1 ? 2097152L
                    : Natsuki.getInstance().getConfig().PACKET.NBT.maxNbtSize)));
      } catch (IOException e) {
        return null;
      }
    }
  ...
  public void a(ItemStack itemstack) {
    if (itemstack == null
        || itemstack.getItem() == null) { // CraftBukkit - NPE fix itemstack.getItem()
      this.writeShort(-1);
    } else {
  ...
  //TAKI MALY SYF JAKBY KTOS WYLACZYL LIMIT XD
  private boolean isValid(final NBTTagCompound compound) {
    if (compound == null) {
      return true;
    }

    if (compound.map == null) {
      return true;
    }

    if (compound.map.size() > 1000) {
      return false;
    }
    ...
    return compound.map.values().stream().filter(tag -> tag instanceof NBTTagString)
        .noneMatch(nbtBase -> nbtBase.a_().length() > 300);
  }
  ...
  public PacketDataSerializer a(String s) {
    ...
    if (abyte.length > 32767) { //32767
      throw new EncoderException(
          "String too big (was " + s.length() + " bytes encoded, max " + 12000 + ")");
    } else {
      this.b(abyte.length);
      this.writeBytes(abyte);
      return this;
