...
public class TileEntityChest extends TileEntityContainer implements
    IInventory { // PaperSpigot - remove IUpdatePlayerListBox
  ...
  public void startOpen(EntityHuman entityhuman) {
    if (!entityhuman.isSpectator()) {
      ...
      int oldPower = Math
          .max(0, Math.min(15, this.l)); // CraftBukkit - Get power before new viewer is added
      ...
      if (this.w() == Blocks.TRAPPED_CHEST) {
        ...
        if (oldPower != newPower) {
          org.bukkit.craftbukkit.event.CraftEventFactory
              .callRedstoneChange(world, position.getX(), position.getY(), position.getZ(),
                  oldPower, newPower);
        }
      }
      // CraftBukkit end
  ...
  public void closeContainer(EntityHuman entityhuman) {
    if (!entityhuman.isSpectator() && this.w() instanceof BlockChest) {
      ...
      if (this.l == 0 && this.j > 0.0F || this.l > 0 && this.j < 1.0F) {
        ...
        this.world
            .makeSound(d0, (double) this.getPosition().getY() + 0.5D, d2, "random.chestclosed",
