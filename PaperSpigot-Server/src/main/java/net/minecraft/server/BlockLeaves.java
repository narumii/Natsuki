...
public abstract class BlockLeaves extends BlockTransparent {
      ...
      for (int i1 = -b0; i1 <= b0; ++i1) {
        for (int j1 = -b0; j1 <= b0; ++j1) {
          for (int k1 = -b0; k1 <= b0; ++k1) {
            ...
            if (iblockdata1.getBlock().getMaterial() == Material.LEAVES && !iblockdata1
                .get(BlockLeaves.CHECK_DECAY).booleanValue()) {
  ...
  public void b(World world, BlockPosition blockposition, IBlockData iblockdata, Random random) {
    if (!world.isClientSide) {
      if (iblockdata.get(BlockLeaves.CHECK_DECAY).booleanValue() && iblockdata
          .get(BlockLeaves.DECAYABLE).booleanValue()) {
        byte b0 = 4;
        int i = b0 + 1;
        int j = blockposition.getX();
        int k = blockposition.getY();
        ...
        if (k2 >= 0) {
          world.setTypeAndData(blockposition,
              iblockdata.set(BlockLeaves.CHECK_DECAY, Boolean.valueOf(false)), 4);
        } else {
  ...
  private void e(World world, BlockPosition blockposition) {
    // CraftBukkit start
    LeavesDecayEvent event = new LeavesDecayEvent(world.getWorld()
