...
public class EntitySnowman extends EntityGolem implements IRangedEntity {
  ...
  public EntitySnowman(World world) {
    ...
    this.targetSelector.a(1,
        new PathfinderGoalNearestAttackableTarget(this, EntityInsentient.class, 10, true, false,
            IMonster.d));
  }
  ...
  public void m() {
    ...
    if (!this.world.isClientSide) {
      ...
      if (this.world.getBiome(new BlockPosition(i, 0, k)).a(new BlockPosition(i, j, k)) > 1.0F) {
        this.damageEntity(CraftEventFactory.MELTING,
            1.0F); // CraftBukkit - DamageSource.BURN -> CraftEventFactory.MELTING
      }
      ...
      for (int l = 0; l < 4; ++l) {
        ...
        if (this.world.getType(blockposition).getBlock().getMaterial() == Material.AIR
          ...
          // CraftBukkit start
          org.bukkit.block.BlockState blockState = this.world.getWorld().getBlockAt(i, j, k)
              .getState();
          blockState.setType(CraftMagicNumbers.getMaterial(Blocks.SNOW_LAYER));

          EntityBlockFormEvent event = new EntityBlockFormEvent(this.getBukkitEntity(),
              blockState.getBlock(), blockState);
  ...
  public void a(EntityLiving entityliving, float f) {
    ...
    float f1 = (float) (0.2F * Math.sqrt(d1 * d1 + d3 * d3));
