...
public class EntityIronGolem extends EntityGolem {
  ...
  public EntityIronGolem(World world) {
    ...
    this.targetSelector.a(3,
        new EntityIronGolem.PathfinderGoalNearestGolemTarget(this, EntityInsentient.class, 10,
  ...
  protected void s(Entity entity) {
    if (entity instanceof IMonster && !(entity instanceof EntityCreeper)
        && this.bc().nextInt(20) == 0) {
      this.setGoalTarget((EntityLiving) entity,
          org.bukkit.event.entity.EntityTargetLivingEntityEvent.TargetReason.COLLISION,
          true); // CraftBukkit - set reason
    }
    ...
    if (this.motX * this.motX + this.motZ * this.motZ > 2.500000277905201E-7D
      ...
      if (block.getMaterial() != Material.AIR) {
        this.world.addParticle(EnumParticle.BLOCK_CRACK,
            ...
            ((double) this.random.nextFloat() - 0.5D) * 4.0D, Block.getCombinedId(iblockdata));
      }
    }
  ...
  public boolean a(Class<? extends EntityLiving> oclass) {
    return (!this.isPlayerCreated() || !EntityHuman.class.isAssignableFrom(oclass)) && (
        oclass != EntityCreeper.class && super.a(oclass));
  }
  ...
  public boolean r(Entity entity) {
    ...
    boolean flag = entity
        .damageEntity(DamageSource.mobAttack(this), (float) (7 + this.random.nextInt(15)));
  ...
  protected void dropDeathLoot(boolean flag, int i) {
    ...
    for (k = 0; k < j; ++k) {
      this.a(Item.getItemOf(Blocks.RED_FLOWER), 1,
          (float) BlockFlowers.EnumFlowerVarient.POPPY.b());
    }
  ...
  static class PathfinderGoalNearestGolemTarget<T extends EntityLiving> extends
      PathfinderGoalNearestAttackableTarget<T> {

    public PathfinderGoalNearestGolemTarget(final EntityCreature entitycreature, Class<T> oclass,
        int i, boolean flag, boolean flag1, final Predicate<? super T> predicate) {
