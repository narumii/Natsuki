...
public class EntitySheep extends EntityAnimal {
  ...
  public boolean a(EntityHuman entityhuman) {
    ...
    if (itemstack != null && itemstack.getItem() == Items.SHEARS && !this.isSheared() && !this
        .isBaby()) {
      if (!this.world.isClientSide) {
        // CraftBukkit start
        ...
        for (int j = 0; j < i; ++j) {
          EntityItem entityitem = this
              .a(new ItemStack(Item.getItemOf(Blocks.WOOL), 1, this.getColor().getColorIndex()),
                  1.0F);
  ...
  public static EnumColor a(Random random) {
    ...
    return i < 5 ? EnumColor.BLACK : (i < 10 ? EnumColor.GRAY : (i < 15 ? EnumColor.SILVER
  ...
  private EnumColor a(EntityAnimal entityanimal, EntityAnimal entityanimal1) {
    ...
    ItemStack itemstack = CraftingManager.getInstance()
        .craft(this.bm, ((EntitySheep) entityanimal).world);
    int k;
