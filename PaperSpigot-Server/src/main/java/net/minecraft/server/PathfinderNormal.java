...
public class PathfinderNormal extends PathfinderAbstract {
  ...
  public PathPoint a(Entity entity) {
    ...
    if (this.i && entity.V()) {
      ...
      BlockPosition.MutableBlockPosition blockposition_mutableblockposition = new BlockPosition.MutableBlockPosition(
          MathHelper.floor(entity.locX), i, (int) Math.floor(entity.locZ));

      for (Block block = this.a.getType(blockposition_mutableblockposition).getBlock();
          block == Blocks.FLOWING_WATER || block == Blocks.WATER;
          block = this.a.getType(blockposition_mutableblockposition).getBlock()) {
        ++i;
        blockposition_mutableblockposition
            .c(MathHelper.floor(entity.locX), i, (int) Math.floor(entity.locZ));
      }
    ...
    return this.a(MathHelper.floor(entity.getBoundingBox().a), i,
        (int) Math.floor(entity.getBoundingBox().c));
  }

  public PathPoint a(Entity entity, double d0, double d1, double d2) {
    return this.a(MathHelper.floor(d0 - (double) (entity.width / 2.0F)), (int) Math.floor(d1),
        (int) Math.floor(d2 - (double) (entity.width / 2.0F)));
  }

  public int a(PathPoint[] apathpoint, Entity entity, PathPoint pathpoint, PathPoint pathpoint1,
      float f) {
    int i = 0;
    byte b0 = 0;
  ...
  public static int a(IBlockAccess iblockaccess, Entity entity, int i, int j, int k, int l, int i1,
      int j1, boolean flag, boolean flag1, boolean flag2) {
    boolean flag3 = false;
          ...
          if (block.getMaterial() != Material.AIR) {
            ...
            if (block instanceof BlockMinecartTrackAbstract) { // PaperSpigot - Pathfinder optimizations
              if (!(iblockaccess.getType(blockposition)
                  .getBlock() instanceof BlockMinecartTrackAbstract) && !(iblockaccess
                  .getType(blockposition.down())
                  .getBlock() instanceof BlockMinecartTrackAbstract)) { // PaperSpigot - Pathfinder optimizations
                return -3;
              }
            } else if (!block.b(iblockaccess, blockposition_mutableblockposition) && (!flag1
