...
public class ChunkRegionLoader implements IChunkLoader, IAsyncChunkSaver {
  ...
  protected Object[] a(World world, int i, int j,
      NBTTagCompound nbttagcompound) { // CraftBukkit - return Chunk -> Object[]
    if (!nbttagcompound.hasKeyOfType("Level", 10)) {
      ChunkRegionLoader.a
          .error("Chunk file at " + i + "," + j + " is missing level data, skipping");
      return null;
    } else {
      ...
      if (!nbttagcompound1.hasKeyOfType("Sections", 9)) {
        ChunkRegionLoader.a
            .error("Chunk file at " + i + "," + j + " is missing block data, skipping");
        return null;
      } else {
        ...
        if (!chunk.a(i, j)) {
          ChunkRegionLoader.a.error(
              "Chunk file at " + i + "," + j + " is in the wrong location; relocating. (Expected "
  ...
  public boolean c() {
    if (this.b.isEmpty()) {
      if (this.e) {
        ChunkRegionLoader.a
            .info("ThreadedAnvilChunkStorage ({}): All chunks are saved", this.d.getName());
      }
  ...
  private void b(ChunkCoordIntPair chunkcoordintpair, NBTTagCompound nbttagcompound)
    ...
    DataOutputStream dataoutputstream = RegionFileCache
        .d(this.d, chunkcoordintpair.x, chunkcoordintpair.z);
    ...
    for (int j = 0; j < i; ++j) {
      ...
      if (chunksection != null) {
        ...
        if (flag) {
          ...
          nbttagcompound1
              .setByteArray("SkyLight", new byte[chunksection.getEmittedLightArray().a().length]);
        }
  ...
  private Chunk a(World world, NBTTagCompound nbttagcompound) {
    ...
    for (int k = 0; k < nbttaglist.size(); ++k) {
      ...
      NibbleArray nibblearray1 = nbttagcompound1.hasKeyOfType("Add", 7) ? new NibbleArray(
          nbttagcompound1.getByteArray("Add")) : null;
    ...
    if (nbttaglist1 != null) {
      for (int i2 = 0; i2 < nbttaglist1.size(); ++i2) {
        ...
        if (entity != null) {
          ...
          for (NBTTagCompound nbttagcompound3 = nbttagcompound2;
              nbttagcompound3.hasKeyOfType("Riding", 10);
