...
public class DedicatedServer extends MinecraftServer implements IMinecraftServer {
  ...
  private final List<ServerCommand> l = Collections
      .synchronizedList(Lists.newArrayList()); // CraftBukkit - fix decompile error
  ...
  protected boolean init() throws IOException {
    ...
    thread.start();
    DedicatedServer.LOGGER.info("Starting Natsuki");
    ...
    if (!this.p.a() && !eulaAgreed) { // Spigot
      ...
      DedicatedServer.LOGGER.info(
          "Starting Natsuki server on " + (this.getServerIp().length() == 0 ? "*"
              : this.getServerIp()) + ":" + this.R());
      ...
      if (!NameReferencingFileConverter.a(this.propertyManager)) {
        ...
        this.convertable = new WorldLoaderServer(
        ...
        ga[0] = this.propertyManager.getString("server-ip", "");
        ga[1] = String.valueOf(this.propertyManager.getInt("server-port", 25565));
        this.aB();
        ...
        this.propertyManager
            .setProperty("max-build-height", Integer.valueOf(this.getMaxBuildHeight()));
  ...
  private void startTasks() {
    Executors.newScheduledThreadPool(1)
        .scheduleAtFixedRate(() -> Bukkit.getServer().getOnlinePlayers().forEach(player -> {
  ...
  public EnumDifficulty getDifficulty() {
    return EnumDifficulty
        .getById(this.propertyManager.getInt("difficulty", EnumDifficulty.NORMAL.a()));
  }
  ...
  public void a(MojangStatisticsGenerator mojangstatisticsgenerator) {
    ...
    mojangstatisticsgenerator
        .a("whitelist_count", Integer.valueOf(this.aP().getWhitelisted().length));
  ...
  protected boolean aR() {
    server.getLogger().info(
        "**** Beginning UUID conversion, this may take A LONG time ****"); // Spigot, let the user know whats up!
    boolean flag = false;
    ...
    for (i = 0; !flag && i <= 2; ++i) {
      if (i > 0) {
        DedicatedServer.LOGGER.warn(
            "Encountered a problem while converting the user banlist, retrying in a few seconds");
        this.aU();
      }
    ...
    for (i = 0; !flag1 && i <= 2; ++i) {
      if (i > 0) {
        DedicatedServer.LOGGER.warn(
            "Encountered a problem while converting the ip banlist, retrying in a few seconds");
        this.aU();
      }
    ...
    for (i = 0; !flag3 && i <= 2; ++i) {
      if (i > 0) {
        DedicatedServer.LOGGER.warn(
            "Encountered a problem while converting the whitelist, retrying in a few seconds");
        this.aU();
      }
